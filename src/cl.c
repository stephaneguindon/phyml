...
int Read_Command_Line(option *io, int argc, char **argv)
{
  ...
  if (argc == 1)
    Exit("\n. No argument was passed to the program. Please check the "
         "documentation. \n");
  ...
  do
  {
    ...
    case 64:
    {
      ...
      if (!strcmp(optarg, "gbd"))
        io->rates->model_id = THORNE;
      else if (!strcmp(optarg, "autocorrelated"))
        io->rates->model_id = THORNE;
      else if (!strcmp(optarg, "autocorr"))
        io->rates->model_id = THORNE;
      else if (!strcmp(optarg, "geom"))
        io->rates->model_id = GUINDON;
      else if (!strcmp(optarg, "integrated"))
        io->rates->model_id = GUINDON;
      else if (!strcmp(optarg, "uncorrelated"))
        io->rates->model_id = LOGNORMAL;
      else if (!strcmp(optarg, "uncorr"))
        io->rates->model_id = LOGNORMAL;
      else if (!strcmp(optarg, "lognorm"))
        io->rates->model_id = LOGNORMAL;
      else if (!strcmp(optarg, "clock"))
        io->rates->model_id = STRICTCLOCK;
      else if (!strcmp(optarg, "strictclock"))
        io->rates->model_id = STRICTCLOCK;
      else if (!strcmp(optarg, "strict_clock"))
        io->rates->model_id = STRICTCLOCK;
      else
      {
        PhyML_Printf("\n. rate_model should be 'autocorrelated', "
                     "'uncorrelated', 'integrated' or 'clock'.");
        Exit("\n");
      }
      Free(s);
      break;
    ...
    case 6:
    {
      ...
      {
        PhyML_Printf("\n. The NNI option is deprecated. PhyML now uses a mix "
                     "of SPRs and NNIs.");
      ...
      {
        PhyML_Printf("\n. The BEST option is deprecated. PhyML now uses a mix "
                     "of SPRs and NNIs.");
    ...
    case 7:
    {
      if (!strcmp(optarg, "nt"))
      {
        ...
        if ((io->mod->whichmodel == LG) || (io->mod->whichmodel == WAG) ||
            (io->mod->whichmodel == DAYHOFF) || (io->mod->whichmodel == JTT) ||
            (io->mod->whichmodel == BLOSUM62) ||
            (io->mod->whichmodel == MTREV) || (io->mod->whichmodel == RTREV) ||
            (io->mod->whichmodel == CPREV) || (io->mod->whichmodel == DCMUT) ||
            (io->mod->whichmodel == VT) || (io->mod->whichmodel == MTMAM) ||
            (io->mod->whichmodel == MTART) || (io->mod->whichmodel == HIVW) ||
            (io->mod->whichmodel == HIVB) || (io->mod->whichmodel == AB) ||
      ...
      else if (!strcmp(optarg, "aa"))
      {
        ...
        if ((io->mod->whichmodel == JC69) || (io->mod->whichmodel == K80) ||
            (io->mod->whichmodel == F81) || (io->mod->whichmodel == HKY85) ||
      ...
      {
        char choix;
        PhyML_Printf("\n. Unknown argument to -d option: please use `nt' for "
                     "DNA or `aa' for Amino-Acids\n");
        PhyML_Printf("\n. Type any key to exit.\n");
        if (!scanf("%c", &choix)) Exit("\n");
        Exit("\n");
      }
    ...
    case 5:
    {
      ...
      if (!isalpha(optarg[0]))
      {
        ...
        else // GTR model with user defined relative rates
        {
          phydbl      v;
          int         n_rr;
          const char *d   = ",";
      ...
      else
      {
        ...
        {
          PhyML_Printf("\n. The model name is incorrect. Please see the "
                       "documentation.\n");
          Exit("\n");
        }
        Free(s);
    ...
    case 14:
    {
      if ((strcmp(optarg, "e") == 0) || (strcmp(optarg, "E") == 0) ||
          (strcmp(optarg, "estimated") == 0) ||
          (strcmp(optarg, "ESTIMATED") == 0))
      {
        io->mod->ras->alpha->optimize = YES;
      }
    ...
    case 10:
    {
      ...
      {
        char choix;
        PhyML_Printf("\n. Branch test value must be a positive integer for "
                     "bootstrap, or between -1 and -4 for aLRT branch test\n");
        PhyML_Printf("\n. Type any key to exit.\n");
        if (!scanf("%c", &choix)) Exit("\n");
        Exit("\n");
      }
      else
      {
        if ((int)String_To_Dbl(optarg) > 0)
        {
          ...
          {
            char choix;
            PhyML_Printf("\n. Bootstrap option is not allowed with multiple "
                         "data sets\n");
            PhyML_Printf("\n. Type any key to exit.\n");
            if (!scanf("%c", &choix)) Exit("\n");
            Exit("\n");
    ...
    case 12:
    {
      ...
      {
        char choix;
        PhyML_Printf("\n. Unknown argument to -c option: the number of rate "
    ...
    case 'f':
    {
      // Won't work if ns is not set beforehand, i.e., if -d comes after -f
      // in the command line
      ...
      if (!strcmp(optarg, "e")) // empirical state frequencies
      {
        ...
        io->mod->e_frq->pi->optimize          = NO;
        io->mod->e_frq->pi_unscaled->optimize = NO;
      }
      else if (!strcmp(optarg, "o")) // ML estimation
      {
        io->mod->e_frq->type                  = ML;
        io->mod->e_frq->pi->optimize          = YES;
        io->mod->e_frq->pi_unscaled->optimize = YES;
      }
      ...
      else if (!isalpha(optarg[0]))
      {
        ...
        if (io->datatype == NT)
        {
          ...
          sum = (io->mod->e_frq->user_b_freq->v[0] +
    ...
    case 11:
    {
      ...
      if ((io->mod->whichmodel != JC69) && (io->mod->whichmodel != F81) &&
          (io->mod->whichmodel != GTR))
      {
        if ((strcmp(optarg, "e") == 0) || (strcmp(optarg, "E") == 0) ||
            (strcmp(optarg, "estimated") == 0) ||
        ...
        {
          io->mod->kappa->v        = 4.0;
          io->mod->kappa->optimize = YES;
          if (io->mod->whichmodel == TN93)
          {
            io->mod->lambda->optimize = YES;
          }
        }
        else
        {
          ...
          if (io->mod->whichmodel == TN93)
          {
            ...
            if ((TsTvPur < .0) || (TsTvPyr < .0))
            {
              ...
              PhyML_Fprintf(stderr,
                            "\n. The TN93 model requires two ts/tv ratios");
              PhyML_Fprintf(stderr, "\n. The command-line option should look "
                                    "as follows: ... -t 3.5,4.3 ... ");
              PhyML_Fprintf(stderr,
    ...
    case 13:
    {
      ...
      {
        char choix;
        PhyML_Printf("\n. The proportion of invariable site must be a number "
                     "between 0.0 and 1.0\n");
        PhyML_Printf("\n. Type any key to exit.");
        if (!scanf("%c", &choix)) Exit("\n");
        Exit("\n");
    ...
    case 'o':
    {
      if (!strcmp(optarg, "tlr"))
      {
        ...
        io->mod->s_opt->opt_bl_one_by_one = YES;
      ...
      {
        Warn_And_Exit("\n. You can't optimize the topology without adjusting "
                      "branch length too...\n");
      }
      ...
      {
        io->mod->s_opt->opt_topo          = NO;
        io->mod->s_opt->opt_bl_one_by_one = NO;
        io->mod->s_opt->opt_subst_param   = YES;
      }
      ...
      {
        char choix;
        PhyML_Printf("\n. The optimization parameter must be 'tlr' or 'tl' or "
                     "'lr' or 'l' or 'r' or 'n'.");
        PhyML_Printf("\n. Type any key to exit.\n");
        if (!scanf("%c", &choix)) Exit("\n");
        Exit("\n");
  ...
  /* 	{ */
  /* 	  char c; */
  /* 	  PhyML_Printf("\n. WARNING: nucleotide frequencies must be set to 1/4
   * with this model.\n"); */
  ...
  if (io->mod->s_opt->constrained_br_len == YES) io->mod->s_opt->opt_topo = NO;
  ...
  if (io->datatype == AA && io->mod->whichmodel == CUSTOMAA &&
      !io->mod->fp_aa_rate_mat)
  {
    PhyML_Printf(
        "\n. Custom model option with amino-acid requires you to specify a "
        "rate matrix file through the '--aa_rate_file' option.\n");
    Exit("\n");
  }

#if !defined(PHYTIME) && !defined(TEST) && !defined(EVOLVE)
  ...
  {
    PhyML_Fprintf(stderr,
                  "\n. The alignment file '%s' does not seem to exist...",
                  io->in_align_file);
