    ...
    if (io->data)
    {
           ...
           num_tree < io->n_trees; num_tree++)
      {
             ...
             num_rand_tree++)
        {
          ...
          /* Print the tree estimated using the current random (or BioNJ)
           * starting tree */
          ...
          Print_Fp_Out(io->fp_out_stats, t_beg, t_end, tree, io,
                       num_data_set + 1,
                       (tree->mod->s_opt->n_rand_starts > 1) ? (num_rand_tree)
                                                             : (num_tree),
                       YES, io->precision);
        ...
        else if (io->ratio_test)
        {
          /* Launch aLRT */
          if (!io->quiet)
            PhyML_Printf("\n. Compute aLRT branch supports on the most likely "
                         "tree...\n");
...
void M4_Update_Qmat(m4 *m4mod, t_mod *mod)
{
  ...
  {
    DiscreteGamma(m4mod->h_fq->v, m4mod->multipl->v, m4mod->alpha->v,
                  m4mod->alpha->v, m4mod->n_h, mod->ras->gamma_median);
  }
  else if (mod->m4mod->use_cov_free)
  {
    sum = .0;
    for (i = 0; i < mod->m4mod->n_h; i++)
      sum += FABS(mod->m4mod->h_fq_unscaled->v[i]);
    for (i = 0; i < mod->m4mod->n_h; i++)
      mod->m4mod->h_fq->v[i] = FABS(mod->m4mod->h_fq_unscaled->v[i]) / sum;
    ...
    do
    {
      ...
      for (i = 0; i < mod->m4mod->n_h; i++)
      {
        if (mod->m4mod->h_fq->v[i] < 0.01) mod->m4mod->h_fq->v[i] = 0.01;
        if (mod->m4mod->h_fq->v[i] > 0.99) mod->m4mod->h_fq->v[i] = 0.99;
        sum += mod->m4mod->h_fq->v[i];
      }

      for (i = 0; i < mod->m4mod->n_h; i++) mod->m4mod->h_fq->v[i] /= sum;
    } while ((sum > 1.01) || (sum < 0.99));

    /* Make sure the multipliers are centered on 1.0 */
    sum = .0;
    for (i = 0; i < mod->m4mod->n_h; i++)
      sum += FABS(mod->m4mod->multipl_unscaled->v[i]) * mod->m4mod->h_fq->v[i];
    for (i = 0; i < mod->m4mod->n_h; i++)
      mod->m4mod->multipl->v[i] = mod->m4mod->multipl_unscaled->v[i] / sum;
    ...
    sum = 0;
    for (i = 0; i < mod->m4mod->n_h; i++)
      sum += mod->m4mod->multipl->v[i] * mod->m4mod->h_fq->v[i];
  ...
  for (i = 0; i < n_s; i++)
    mod->e_frq->pi->v[i] = m4mod->o_fq[i % n_o] * m4mod->h_fq->v[i / n_o];
      ...
      mod->io->datatype == NT)
  {
    ...
    /* A <-> C */ m4mod->o_rr->v[0] = 1.0;
    /* A <-> G */ m4mod->o_rr->v[1] = kappa2;
    /* A <-> T */ m4mod->o_rr->v[2] = 1.0;
    /* C <-> G */ m4mod->o_rr->v[3] = 1.0;
    /* C <-> T */ m4mod->o_rr->v[4] = kappa1;
  }

  /* Fill in the matrices of nucleotide or amino-acid substitution rates here */
  Update_Qmat_Generic(m4mod->o_rr->v, m4mod->o_fq, m4mod->n_o,
                      m4mod->o_mats[0]);

  /* Print_Square_Matrix_Generic(n_o,m4mod->o_mats[0]); */

  /* Multiply each of these matrices by a relative substitution rate */
  for (i = 1; i < m4mod->n_h; i++)
    For(j, n_o * n_o) m4mod->o_mats[i][j] =
        m4mod->o_mats[0][j] * m4mod->multipl->v[i];
  for (j = 0; j < n_o * n_o; ++j) m4mod->o_mats[0][j] *= m4mod->multipl->v[0];

  for (i = 0; i < n_s * n_s; ++i) mod->r_mat->qmat->v[i] = .0;
    ...
    for (j = i + 1; j < n_s; j++)
    {
      if ((int)(j / n_o) == (int)(i / n_o))
      {
        mod->r_mat->qmat->v[i * n_s + j] =
            m4mod->o_mats[(int)(i / n_o)][(i % n_o) * n_o + j % n_o];
        mod->r_mat->qmat->v[j * n_s + i] = mod->r_mat->qmat->v[i * n_s + j] *
                                           m4mod->o_fq[i % n_o] /
                                           m4mod->o_fq[j % n_o];
      }
    }
  ...
  /* Work out scaling factor such that the  expected number of observed state
     substitution along a branch of length 1 is 1.*/
  mr = .0;
  for (i = 0; i < n_s; i++)
  {
    sum = .0;
    for (j = 0; j < n_s; j++) sum += mod->r_mat->qmat->v[i * n_s + j];
    mr += sum * m4mod->o_fq[i % n_o] * m4mod->h_fq->v[(int)(i / n_o)];
  }
  ...
  Update_Qmat_Generic(m4mod->h_rr, m4mod->h_fq->v, m4mod->n_h, m4mod->h_mat);

  /*   Print_Square_Matrix_Generic(m4mod->n_h,m4mod->h_mat); */

  /* Multiply this matrix by the switching rate */
  For(i, n_h * n_h) m4mod->h_mat[i] *= m4mod->delta->v;
  ...
  for (i = 0; i < n_s; i++)
  {
    for (j = i + 1; j < n_s; j++)
    {
      if ((int)(j / n_o) != (int)(i / n_o))
      {
        ...
        {
          mod->r_mat->qmat->v[i * n_s + j] =
              m4mod->h_mat[(int)(i / n_o) * n_h + (int)(j / n_o)];
          mod->r_mat->qmat->v[j * n_s + i] = mod->r_mat->qmat->v[i * n_s + j] *
                                             m4mod->h_fq->v[(int)(i / n_o)] /
                                             m4mod->h_fq->v[(int)(j / n_o)];
        }
      }
    }
  }
  ...
  for (i = 0; i < n_s; i++)
  {
    ...
    for (j = 0; j < n_s; j++)
    {
      if (j != i) sum += mod->r_mat->qmat->v[i * n_s + j];
    }
    ...
    for (i = 0; i < tree->n_otu; i++)
    {
      for (j = 1; j < tree->mod->m4mod->n_h; j++)
      {
        for (k = 0; k < tree->mod->m4mod->n_o; k++)
        {
          tree->a_nodes[i]
              ->b[0]
              ->p_lk_rght[curr_site * dim1 + 0 * dim2 + j * dim3 + k] =
              tree->a_nodes[i]
                  ->b[0]
                  ->p_lk_rght[curr_site * dim1 + 0 * dim2 + 0 * dim3 + k];

          printf("\n() i=%d plk=%f", curr_site * dim1 + 0 * dim2 + j * dim3 + k,
                 tree->a_nodes[i]->b[0]->p_lk_rght[curr_site * dim1 + 0 * dim2 +
                                                   j * dim3 + k]);
        ...
        for (k = 0; k < tree->mod->m4mod->n_o; k++)
          for (l = 1; l < tree->mod->ras->n_catg; l++)
            tree->a_nodes[i]
                ->b[0]
                ->p_lk_rght[curr_site * dim1 + l * dim2 + j * dim3 + k] =
                tree->a_nodes[i]
                    ->b[0]
                    ->p_lk_rght[curr_site * dim1 + 0 * dim2 + j * dim3 + k];
      ...
      for (j = 1; j < tree->mod->m4mod->n_h; j++)
      {
        for (k = 0; k < tree->mod->m4mod->n_o; k++)
        {
          tree->a_nodes[i]->b[0]->p_lk_tip_r[curr_site * dim2 + j * dim3 + k] =
              tree->a_nodes[i]
                  ->b[0]
                  ->p_lk_tip_r[curr_site * dim2 + 0 * dim3 + k];
...
phydbl ****M4_Integral_Term_On_One_Edge(t_edge *b, t_tree *tree)
{
  ...
  for (g = 0; g < tree->mod->ras->n_catg; g++)
  {
    ...
    for (j = 0; j < ns; j++)
    {
      ...
      for (k = 0; k < ns; k++)
        integral[g][j][k] = (phydbl *)mCalloc(ns, sizeof(phydbl));
    }
  }
  ...
  for (i = 0; i < step; i++)
  {
    for (g = 0; g < tree->mod->ras->n_catg; g++)
    {
      PMat(((phydbl)(i + 0.5) / step) * b->l->v *
               tree->mod->ras->gamma_rr->v[g],
           tree->mod, g * ns * ns, P1, NULL);
      PMat(((phydbl)(step - i - 0.5) / step) * b->l->v *
               tree->mod->ras->gamma_rr->v[g],
           tree->mod, g * ns * ns, P2, NULL);
      ...
      for (j = 0; j < ns; j++)
      {
        for (k = 0; k < ns; k++)
        {
          for (l = 0; l < ns; l++)
          {
            /* integral[g][j][k][l] += P1[g][j][k] * P2[g][j][l]  /
             * ((phydbl)(step)); */
            integral[g][j][k][l] += P1[g * ns * ns + j * ns + k] *
                                    P2[g * ns * ns + j * ns + l] /
                                    ((phydbl)(step));
          }
        }
                                    ...
                                    phydbl *postprob, t_tree *tree)
{
  ...
  n_h = tree->mod->m4mod
            ->n_h; /* number of classes, i.e., number of hidden states */
  ...
  if (b->rght->tax)
  {
          ...
          for (k = 0; k < tree->mod->ns; k++)
          {
            for (l = 0; l < tree->mod->ns; l++)
            {
              postprob[i] +=
                  ...
                  (1. / site_lk) * tree->mod->ras->gamma_r_proba->v[g] *
                  tree->mod->m4mod->h_fq->v[i] * tree->mod->m4mod->o_fq[j] *
    ...
    /* TO DO */
    for (i = 0; i < n_h; i++)
      postprob[i] *= exp(b->sum_scale_left[tree->curr_site]);
  }
  else
  {
    sum = .0;
    ...
    for (i = 0; i < n_h; i++)
      postprob[i] *= exp(b->sum_scale_left[tree->curr_site] +
                         b->sum_scale_rght[tree->curr_site]);
  }
...
phydbl ***M4_Compute_Proba_Hidden_States_On_Edges(t_tree *tree)
{
  ...
  {
    post_probs[i] = (phydbl **)mCalloc(tree->data->n_pattern, sizeof(phydbl *));
    for (tree->curr_site = 0; tree->curr_site < tree->data->n_pattern;
         tree->curr_site++)
      post_probs[i][tree->curr_site] =
          (phydbl *)mCalloc(tree->mod->m4mod->n_h, sizeof(phydbl));
  }
  ...
  */
  for (i = 0; i < 2 * tree->n_otu - 3; ++i)
  {
    ...
    for (tree->curr_site = 0; tree->curr_site < tree->data->n_pattern;
         tree->curr_site++)
      M4_Post_Prob_H_Class_Edge_Site(tree->a_edges[i], integral,
                                     post_probs[i][tree->curr_site], tree);
...
/* Estimate the (posterior) mean relative rate of substitution on each branch
   ...
   for each edge. The corresponding trees are printed in a postscript file. Tree
   0 is the tree with posterior mean rates averaged over the sites. The
   following trees have posterior mean rates computed for each site.
*/
void M4_Compute_Posterior_Mean_Rates(phydbl ***post_probs, t_tree *tree)
{
  ...
  For(i, 2 * tree->n_otu - 3) mean_post_probs[i] =
      (phydbl *)mCalloc(tree->mod->m4mod->n_h, sizeof(phydbl));
  ...
  for (patt = 0; patt < tree->data->n_pattern; patt++)
  {
    if (!Is_Invar(patt, 1, NT, tree->data))
    {
      For(br, 2 * tree->n_otu - 3)
      {
        ...
        /* /\* 	      Add weight on each category, weight is proportional to the
         * corresponding posterior probability *\/ */
        /* 	      for(rcat=0;rcat<tree->mod->m4mod->n_h;rcat++) */
        /* 		{ */
        /* 		  mean_post_probs[br][rcat] +=
         * post_probs[br][patt][rcat] * tree->data->wght[patt]; */
        /* 		} */
  ...
  for (br = 0; br < 2 * tree->n_otu - 3; ++br)
  {
    for (rcat = 0; rcat < tree->mod->m4mod->n_h; rcat++)
    {
      mrr[br] += mean_post_probs[br][rcat] * tree->mod->m4mod->multipl->v[rcat];
    }
  ...
  for (rcat = 0; rcat < tree->mod->m4mod->n_h; rcat++)
    PhyML_Fprintf(tree->io->fp_out_stats, "%2.4f ",
                  tree->mod->m4mod->multipl[rcat]);
  PhyML_Fprintf(tree->io->fp_out_stats, "\n");
  For(br, 2 * tree->n_otu - 3)
  {
    for (rcat = 0; rcat < tree->mod->m4mod->n_h; rcat++)
    ...
    /*       PhyML_Fprintf(tree->io->fp_out_stats," -- %f -> %f x %f =
     * %f",mrr[br],tree->a_edges[br]->l->v,mrr[br],tree->a_edges[br]->l->v*mrr[br]);
     */

    PhyML_Fprintf(tree->io->fp_out_stats, " mrr=%f ", mrr[br]);

    if (mrr[br] > 1.)
      PhyML_Fprintf(tree->io->fp_out_stats, "FAST ");
    else
      PhyML_Fprintf(tree->io->fp_out_stats, "SLOW ");

    PhyML_Fprintf(tree->io->fp_out_stats, "%s", tree->a_edges[br]->label[0]);
  ...
  PhyML_Fprintf(tree->io->fp_out_tree,
                "Constrained tree with corrected branch lengths = ");
  s = Write_Tree(tree);
  ...
  DR_Print_Tree_Postscript(tree->ps_page_number++, YES, tree->io->fp_out_ps,
                           tree);
  ...
  for (patt = 0; patt < tree->data->n_pattern; patt++)
  {
    For(br, 2 * tree->n_otu - 3)
    {
      ...
      max_prob = -1.;
      best_r   = -1;
      for (rcat = 0; rcat < tree->mod->m4mod->n_h;
           rcat++) /* For each rate class */
      {
        mrr[br] +=
            post_probs[br][patt][rcat] * tree->mod->m4mod->multipl->v[rcat];
        if (post_probs[br][patt][rcat] > max_prob)
        {
          max_prob = post_probs[br][patt][rcat];
          best_r   = rcat;
      ...
      /* 	  mrr[br] = tree->mod->m4mod->multipl[best_r]; /\* Use the most
       * probable relative rate instead of mean *\/ */
      tree->a_edges[br]->l->v *= mrr[br];
    }

    For(br, 2 * tree->n_otu - 3) mean_br_len[br] +=
        tree->a_edges[br]->l->v * tree->data->wght[patt];

    PhyML_Fprintf(
        tree->io->fp_out_stats,
        "\n. Posterior probabilities site %4d (weight=%d, is_inv=%d)\n", patt,
        tree->data->wght[patt], Is_Invar(patt, 1, NT, tree->data));
    ...
    For(br, 2 * tree->n_otu - 3)
    {
      ...
      if (mrr[br] > 1.01)
        PhyML_Fprintf(tree->io->fp_out_stats, " %s ", "FAST");
      else if (mrr[br] < 0.99)
        PhyML_Fprintf(tree->io->fp_out_stats, " %s ", "SLOW");
      else
        PhyML_Fprintf(tree->io->fp_out_stats, " %s ", "MEDIUM");
      PhyML_Fprintf(tree->io->fp_out_stats, "%s ", tree->a_edges[br]->label[0]);
      PhyML_Fprintf(tree->io->fp_out_stats, "\n");
    }
...
phydbl **M4_Site_Branch_Classification(phydbl ***post_probs, t_tree *tree)
{
  ...
  for (i = 0; i < tree->data->n_pattern; i++)
    best_probs[i] = (phydbl *)mCalloc(2 * tree->n_otu - 3, sizeof(phydbl));
  ...
  for (patt = 0; patt < tree->data->n_pattern; patt++)
  {
    for (br = 0; br < 2 * tree->n_otu - 3; ++br)
    {
      ...
      for (rcat = 0; rcat < tree->mod->m4mod->n_h;
           rcat++) /* For each rate class */
      {
        if (tree->mod->m4mod->multipl->v[rcat] > 1.0)
          post_prob_fast += post_probs[br][patt][rcat];
        else
          post_prob_slow += post_probs[br][patt][rcat];
      }

      best_probs[patt][br] = (post_prob_fast > post_prob_slow)
                                 ? (post_prob_fast)
                                 : (post_prob_slow);

      tree->a_edges[br]->label = Make_Label();

      /* 	  if((post_prob_fast > post_prob_slow) && (best_probs[patt][br]
       * > 0.95)) */
      /* 	    strcpy(tree->a_edges[br]->labels[tree->a_edges[br]->n_labels],"FASTER");
       */
      /* 	  else if((post_prob_fast < post_prob_slow) &&
       * (best_probs[patt][br] > 0.95)) */
      ...
      if (post_prob_fast > post_prob_slow)
        strcpy(tree->a_edges[br]->label->val, "FASTER");
      else
        strcpy(tree->a_edges[br]->label->val, "SLOWER");

      tree->a_edges[br]->label = tree->a_edges[br]->label->next;
    }
  }
  return best_probs;
}
...
void M4_Site_Branch_Classification_Experiment(t_tree *tree)
{
  ...
  true_rclass =
      (short int **)mCalloc(tree->data->init_len, sizeof(short int *));
  est_rclass = (short int **)mCalloc(tree->data->init_len, sizeof(short int *));

  for (i = 0; i < tree->data->init_len; i++)
  {
    true_rclass[i] =
        (short int *)mCalloc(2 * tree->n_otu - 3, sizeof(short int));
    est_rclass[i] =
        (short int *)mCalloc(2 * tree->n_otu - 3, sizeof(short int));
  }
  ...
  {
    do tree->a_edges[j]->label = tree->a_edges[j]->label->prev;
    while (tree->a_edges[j]->label->prev != NULL);
  }
    ...
    for (i = 0; i < cpy_data->init_len; i++)
    {
      for (j = 0; j < 2 * tree->n_otu - 3; ++j)
      {
        if (!strcmp(tree->a_edges[j]->label->val, "FASTER"))
        {
          true_rclass[i][j] = 1;
        }
        else if (!strcmp(tree->a_edges[j]->label->val, "SLOWER"))
        {
          true_rclass[i][j] = 0;
        }
        else
      ...
      tree->a_edges[j]->label = tree->a_edges[j]->label->next;
    }

    for (j = 0; j < 2 * tree->n_otu - 3; ++j) Free_Label(tree->a_edges[j]->label);
    ...
    */
    ...
    /* Classify branches */
    best_probs = M4_Site_Branch_Classification(
        M4_Compute_Proba_Hidden_States_On_Edges(tree), tree);
    ...
    for (i = 0; i < tree->data->init_len; i++)
    {
      for (j = 0; j < 2 * tree->n_otu - 3; ++j)
      {
        ...
        else if (!strcmp(tree->a_edges[j]->label->val, "UNKNOWN"))
        {
          est_rclass[i][j] = -1;
        }
        else
    ...
    for (i = 0; i < tree->data->init_len; i++)
    {
      ...
      {
        /* 	  PhyML_Printf("\n. Edge %3d %4d %4d -
         * %f",j,true_rclass[i][j],est_rclass[i][j],best_probs[i][j]); */
        if (est_rclass[i][j] == -1)
        {
          unknown += 1.;
        }
    ...
    PhyML_Printf("\n. correct_class = %f mis_class = %f unknown = %f",
                 correct_class, mis_class, unknown);
...
void M4_Scale_Br_Len(t_tree *tree)
{
  ...
  for (i = 0; i < tree->mod->m4mod->n_h; i++)
  {
    ...
    {
      if (j != i)
        mrs += tree->mod->m4mod->h_fq->v[i] *
               tree->mod->m4mod->h_mat[i * tree->mod->m4mod->n_h + j];
    }
  }

  /* (2) scale_fact = (1 + delta x mrs) */
  scale_fact = 1.0 + tree->mod->m4mod->delta->v * mrs;
...
void M4_Detect_Site_Switches_Experiment(t_tree *tree)
{
  ...
  For(i, 2 * tree->n_otu - 3) nocov_bl[i] =
      tree->a_edges[i]->l->v; /* Record branch lengths */
  for (i = 0; i < tree->data->n_pattern; i++)
    site_lnl_nocov[i] = tree->cur_site_lk[i];
  ...
  For(i, 2 * tree->n_otu - 3) cov_bl[i] =
      tree->a_edges[i]->l->v; /* Record branch lengths */
  for (i = 0; i < tree->data->n_pattern; i++)
    site_lnl_cov[i] = tree->cur_site_lk[i];
  Print_Lk(tree, "[LnL under switching substitution model]");

  PhyML_Printf("\n");
  for (i = 0; i < tree->data->n_pattern; i++)
    PhyML_Printf("TRUTH %f %f\n", site_lnl_nocov[i], site_lnl_cov[i]);
...
m4 *M4_Copy_M4_Model(t_mod *ori_mod, m4 *ori_m4mod)
{
  ...
  {
    M4_Make_Complete(cpy_m4mod->n_h, cpy_m4mod->n_o, cpy_m4mod);
    ...
    cpy_m4mod->n_o = ori_m4mod->n_o;
    for (i = 0; i < n_h; i++)
      For(j, n_o * n_o) cpy_m4mod->o_mats[i][j] = ori_m4mod->o_mats[i][j];
    for (i = 0; i < n_h; i++) cpy_m4mod->multipl[i] = ori_m4mod->multipl[i];
    for (i = 0; i < n_h; i++)
      cpy_m4mod->multipl_unscaled[i] = ori_m4mod->multipl_unscaled[i];
    ...
    for (i = 0; i < n_h; i++)
      cpy_m4mod->h_fq_unscaled[i] = ori_m4mod->h_fq_unscaled[i];
    cpy_m4mod->delta->v = ori_m4mod->delta->v;
    cpy_m4mod->alpha->v = ori_m4mod->alpha->v;
  }
