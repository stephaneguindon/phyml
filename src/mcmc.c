  ...
  do
    {
      ...
      if(t0_new > t_min-eps && t0_new < t_max) 
        {
          ...
          if(tree->eval_alnL == YES && tree->rates->model_id == STRICTCLOCK)
            {              
              new_lnL_seq = Lk(tree->e_root,tree);
            }
        }

      if(tree->eval_alnL == YES) new_lnL_seq = rate_failed ? UNLIKELY : cur_lnL_seq;
      ...
      /* if(tree->aux_tree != NULL) */
        /* { */
          /* PhyML_Printf("\n."); */
          /* PhyML_Printf("\n. %c [%4d] \n. t: %1f->%1f t1:%f t2: %f \n. r1: %10f->%10f r2: %10f->%10f \n. R: %12f alnL:%10f->%10f[%d] \n. tlnL: %10f->%10f[%d] \n. glnL: %10f->%10f[%d] \n. rlnL: %10f->%10f[%d] \n. tune: %10f hr: %f [%f,%f] [%f,%f,%f] ratio: %10f K: %10f rate_failed: %d", */
...
void MCMC_PHYREX_Sigsq(t_tree *tree, int print)
{
  ...
  else if((RRW_Is_Rw(tree->mmod) == YES || IBM_Is_Ibm(tree->mmod) == YES) && tree->mmod->sampling_scheme == SPATIAL_SAMPLING_DETECTION)
   {      
      phydbl cur_sigsq,new_sigsq;
...
void MCMC_PHYREX_Sigsq_Scale(t_tree *tree, int print)
{
  ...
  if((RRW_Is_Rw(tree->mmod) == NO && IBM_Is_Ibm(tree->mmod) == NO) ||
  ...
  if(tree->times->model_id == COALESCENT &&
     ...
     tree->mmod->sampling_scheme == SPATIAL_SAMPLING_SURVEY)
    {
      ...
      tree->mcmc->run_move[tree->mcmc->num_move_time_neff]++;
          ...
          (RRW_Is_Rw(tree->mmod) == YES || IBM_Is_Ibm(tree->mmod) == YES)  &&
...
void MCMC_PHYREX_Indel_Disk(t_tree *tree)
{
  ...
  if(RRW_Is_Rw(tree->mmod) == YES || IBM_Is_Ibm(tree->mmod) == YES) return;  
  if(tree->mmod->use_locations == NO) return;
...
void MCMC_PHYREX_Delete_Disk(phydbl hr, int n_delete_disks, phydbl cur_lbda, phydbl cur_mu, phydbl cur_rad, t_tree *tree)
{
  ...
  assert(RRW_Is_Rw(tree->mmod) == NO && IBM_Is_Ibm(tree->mmod) == NO);
...
void MCMC_PHYREX_Node_Times_Pre(t_ldsk *a_ldsk, t_ldsk *d_ldsk, t_tree *tree, int print)
{
  ...
  if(tree->eval_alnL == YES) new_alnL = rate_failed ? UNLIKELY : cur_alnL;
      ...
      for(i=0;i<tree->mmod->n_dim;i++)
        {
          ...
          else if(RRW_Is_Rw(tree->mmod) == YES || IBM_Is_Ibm(tree->mmod) == YES)
            {
      ...
      if(tree->eval_glnL == YES)
        {
          ...
          /*     for(int i=0;i<10;++i) hr += IBM_Sample_Velocities_And_Locations(permut,tree); */
  ...
  if(IBM_Is_Ibm(tree->mmod) == YES)
    {
      ...
      /*             /\* m = exp(K*(Uni()-.5)); *\/ */
      /*             /\* n->ldsk->veloc->deriv[j] *= m; *\/ */
      /*             /\* hr += log(m); *\/ */

      /*             n->ldsk->veloc->deriv[j] = Rnorm(n->ldsk->veloc->deriv[j],1.0); */
      ...
      /*             /\*   { *\/ */
      /*             /\*     PhyML_Printf("\n. VELOC -- Node %3d(%p) anc %3d(%p) tip? %d root? %d ratio: %14f sigsq: %f sigsq_scale: %f deriv: %f dt: %f", *\/ */
      /*             /\*                  n->num, *\/ */
      /*             /\*                  n->ldsk, *\/ */
      /*             /\*                  n->anc->num, *\/ */
      /*             /\*                  n->anc->ldsk, *\/ */
      ...
      /*             /\*                  tree->mmod->sigsq[j], *\/ */
      /*             /\*                  tree->mmod->sigsq_scale[n->num], *\/ */
      /*             /\*                  n->ldsk->veloc->deriv[j], *\/ */
      /*             /\*                  fabs(tree->times->nd_t[n->num] - tree->times->nd_t[n->v[0]->num])); *\/ */
      ...
      /*             /\* n->ldsk->coord->lonlat[j] = Rnorm(n->ldsk->coord->lonlat[j],1.0); *\/ */
