                               ...
                               t_edge *branch, t_tree *tree, supert_tree *stree)
{
  ...
  if(new_val < lim_sup && new_val > lim_inf)
    {
      *val = new_val;
      ...
      if(prior_func) new_lnPrior = (*prior_func)(branch,tree,stree); 
      if(like_func)  new_lnLike  = (*like_func)(branch,tree,stree);  
    }
  else
    {
      new_lnPrior = new_lnLike = UNLIKELY;
    }
  
  if(prior_func) ratio += (new_lnPrior - cur_lnPrior); 
  if(like_func) ratio += (new_lnLike - cur_lnLike);  
  ...
  if(print == YES)
    PhyML_Printf("\n. cur_val: %f new_val: %f curPrior: %f newPrior: %f curLike: %f newLike: %f alpha: %f K: %f",
                 cur_val,new_val,
                 cur_lnPrior,new_lnPrior,
                 cur_lnLike,new_lnLike,
                 alpha,K);
...
void MCMC_Clock_R(t_tree *tree)
{
  ...
  phydbl new_lnP_rates, cur_lnP_rates;
  ...
  cur_lnP_rates  = tree->rates->c_lnP;
  new_lnP_rates  = UNLIKELY;
  ...
  if(new_clock_r > min && new_clock_r < max)
    {      
      ...
      if(tree->eval_rlnL == YES)
        {
          new_lnL_rates = RATES_Lk(tree);
          new_lnP_rates = RATES_Prior(tree);
        }
    }
  ...
  if(tree->eval_rlnL == YES)
    {
      ...
      ratio += (new_lnP_rates - cur_lnP_rates);
    }
  ...
  if(u > alpha) /* Reject */
    {
      ...
      tree->rates->c_lnP   = cur_lnP_rates;
      RATES_Update_Edge_Lengths(tree);
    }
  else
    {
...
{
  if(tree->eval_alnL == NO || tree->eval_rlnL == NO) return;
...
/* ////////////////////////////////////////////////////////////// */
/* ////////////////////////////////////////////////////////////// */
/* Shrink or expand edge-specific relative rates of substitution */
#if (defined PHYREX || PHYTIME)
void MCMC_Rates_Shrink(t_tree *tree)
{
  ...
  phydbl r; /* Reference value for relative rates (set to 1.0 usually) */
  ...
  r            = 1.0;
  K            = tree->mcmc->tune_move[tree->mcmc->num_move_rates_shrink]; /* parameter of gamma distribution for selecting value of rate multiplier */
  ratio        = 0.0;
  ...
  for(i=0;i<2*tree->n_otu-2;++i)
    {
      ...
      tree->rates->br_r[tree->a_nodes[i]->num] = r + mult*(tree->rates->br_r[tree->a_nodes[i]->num] - r);

      if(tree->rates->br_r[tree->a_nodes[i]->num] < tree->rates->min_rate ||
         tree->rates->br_r[tree->a_nodes[i]->num] > tree->rates->max_rate)
        {
  ...
  ratio += log(Dgamma(1./mult,1./K,K) / Dgamma(mult,1./K,K));
  ...
  /* PhyML_Printf("\n. mult: %6f lnL_seq: %12f -> %12f lnL_rate: %12f -> %12f alpha: %6f", */
  /*              mult, */
  /*              cur_lnL_seq,new_lnL_seq, */
  /*              cur_lnL_rate,new_lnL_rate, */
  /*              alpha); */
  ...
  if(u > alpha)
    ...
    {
      tree->mcmc->acc_move[tree->mcmc->num_move_rates_shrink]++;
    }
...
#ifdef PHYREX
  /* if(tree->mmod->safe_phyrex == YES) PHYREX_Update_Disk_Times_Given_Nodes(tree); */
#endif
}
#endif
...
void MCMC_Time_Recur(t_node *a, t_node *d, int traversal, t_tree *tree)
{
  ...
  if(tree->eval_tlnL == YES) ratio += (new_lnL_time - cur_lnL_time);
...
void MCMC_Root_Time(t_tree *tree, int print)
{
  ...
  phydbl t2,t1;
  t_node *v2,*v1;
  int move_num,iter,n_tot_iter;
  t_node *root;

  if(tree->mod->s_opt->opt_node_ages == NO) return;
  if(tree->eval_tlnL == NO) return;
  ...
  iter = 0;
  n_tot_iter = 1+(int)(0.1*tree->n_otu);
  ...
  do
    {
      ...
      t_min = t0_cur - 0.1 * (tree->young_disk->time - t0_cur);
      t0_new = Uni()*(t_max - t_min) + t_min;
      hr -= log(1./(t_max - (t0_cur - 0.1 * (tree->young_disk->time - t0_cur))));
      hr += log(1./(t_max - (t0_new - 0.1 * (tree->young_disk->time - t0_new))));
      ...
      if(tree->eval_tlnL == YES) ratio += (new_lnL_time - cur_lnL_time);
#ifdef PHYREX
      if(tree->eval_glnL == YES && tree->mmod->use_locations == YES) ratio += (new_lnL_loc - cur_lnL_loc);
#endif
      ratio += hr;
...
void MCMC_Updown_T_Cr(t_tree *tree)
{
  ...
  */
  ...
  phydbl K,mult,u,alpha,ratio;
  phydbl cur_lnL_rate,new_lnL_rate;
  phydbl cur_lnP_rate,new_lnP_rate;
  phydbl cur_lnL_time,new_lnL_time;
  phydbl cur_lnL_seq,new_lnL_seq;
  phydbl floor;
  int n_nodes;
  ...
  cur_lnP_rate = tree->rates->c_lnP;
  new_lnP_rate = UNLIKELY;
  ...
  */
  ...
  if(tree->eval_rlnL == YES)
    {
      ratio += (new_lnL_rate - cur_lnL_rate);
      ratio += (new_lnP_rate - cur_lnP_rate);
    }
  if(tree->eval_tlnL == YES) ratio += (new_lnL_time - cur_lnL_time);
...
void MCMC_Updown_T_Br(t_tree *tree)
{
  ...
  for(i=0;i<2*tree->n_otu-2;++i)
    {
      ...
      tree->rates->br_r[tree->a_nodes[i]->num] /= mult;
  ...
  if(tree->eval_alnL == YES) new_lnL_seq  = Lk(NULL,tree);
  if(tree->eval_rlnL == YES) new_lnL_rate = RATES_Lk(tree);
  if(tree->eval_glnL == YES) new_lnL_geo  = LOCATION_Lk(NULL,tree);
  if(tree->eval_tlnL == YES) new_lnL_time = TIMES_Lk(tree);

  int n = tree->n_otu;
  ratio += (2.*n-2.-2.) * log(1./mult); // HR for rates
  ratio += (n-1.-2.) * log(mult); // HR for times
  ratio += 2.*log(mult); // Ratio of proposal densities
  ...
  /* PhyML_Printf("\n. mult: %f new_lnL_seq: %f cur_lnL_seq: %f new_lnL_rate: %f cur_lnL_rate: %f new_lnL_time: %f cur_lnL_time: %f", */
  /*              mult, */
  /*              new_lnL_seq,cur_lnL_seq, */
  /*              new_lnL_rate,cur_lnL_rate, */
  /*              new_lnL_time,cur_lnL_time); */
...
void MCMC_Randomize_Locations(t_tree *tree)
{
  ...
  if(tree->mmod == NULL) return;
  
  MCMC_Randomize_Locations_Pre(tree->n_root,tree->n_root->v[1],tree);
  MCMC_Randomize_Locations_Pre(tree->n_root,tree->n_root->v[2],tree);

  for(i=0;i<tree->mmod->n_dim;++i)
    {
      tree->n_root->ldsk->coord->lonlat[i] = Rnorm(0.5*(tree->n_root->v[1]->ldsk->coord->lonlat[i] + tree->n_root->v[2]->ldsk->coord->lonlat[i]),
                                                   0.01*(tree->n_root->v[1]->ldsk->coord->lonlat[i] + tree->n_root->v[2]->ldsk->coord->lonlat[i]));
    }
}
...
void MCMC_Randomize_Locations_Pre(t_node *a, t_node *d, t_tree *tree)
{
  if(d->tax == YES) return;
  else
    {
      ...
      for(i=0;i<3;++i)
        {
          if(d->v[i] != a && !(a == tree->n_root && d->b[i] == tree->e_root))
            {
              MCMC_Randomize_Locations_Pre(d,d->v[i],tree);
              if(v1 == NULL) v1 = d->v[i];
              else v2 = d->v[i];
            }
        }
      ...
      for(i=0;i<tree->mmod->n_dim;++i)
        {
          d->ldsk->coord->lonlat[i] = Rnorm(0.5*(v1->ldsk->coord->lonlat[i] + v2->ldsk->coord->lonlat[i]),
                                            0.01*(v1->ldsk->coord->lonlat[i] + v2->ldsk->coord->lonlat[i]));
        }
    }  
}
...
void MCMC_Randomize_Covarion_Rates(t_tree *tree)
{
  ...
  for(i=0;i<tree->mod->m4mod->n_h;i++)
    {
      tree->mod->m4mod->multipl_unscaled->v[i] = (phydbl)i+1.;
      tree->mod->m4mod->h_fq_unscaled->v[i] = Uni()*(100.-0.01) + 0.01;
    }
}
...
void MCMC_Randomize_Covarion_Switch(t_tree *tree)
{
  ...
  tree->mod->m4mod->delta->v = Uni()*(10.-0.01)+0.01;
}
...
void MCMC_Randomize_Sigsq_Scale(t_tree *tree)
{
  ...
  if(tree->mmod->model_id == RRW_GAMMA ||
     ...
     tree->mmod->model_id == RIBM ||
     tree->mmod->model_id == RIWNc ||
     tree->mmod->model_id == RIWNu)
    {
      phydbl sd;
...
void MCMC_Randomize_Sigsq(t_tree *tree)
{
  ...
  for(i=0;i<tree->mmod->n_dim;++i)
    {
      switch(tree->mmod->rw_prior_distrib)
        {
        case EXPONENTIAL_PRIOR:
          {
            tree->mmod->sigsq[i] = Rexp(1./tree->mmod->rw_prior_mean);
            break;
          }
        case NORMAL_PRIOR:
          {
            tree->mmod->sigsq[i] = Rnorm(tree->mmod->rw_prior_mean,
  ...
  if(VELOC_Is_Integrated_Velocity(tree->mmod) == YES)
    {
      ...
      for(j=0;j<tree->mmod->n_dim;++j)
        {
          mean = tree->mmod->rw_root_mean[VELOCITY];          
        //   var = tree->mmod->rw_root_var[VELOCITY];
          var = 0.1;
          
          tree->n_root->ldsk->veloc->deriv[j] = Rnorm(mean,sqrt(var));
          
        //   tree->n_root->ldsk->veloc->deriv[j] = 0.0; // Using a large initial root velocity -> PhyREX cannot sample root velocity properly
  ...
  for(j=0;j<tree->mmod->n_dim;++j)
    {
      ...
      if(d->ldsk->veloc->deriv[j] < tree->mmod->min_veloc) d->ldsk->veloc->deriv[j] = tree->mmod->min_veloc;
      if(d->ldsk->veloc->deriv[j] > tree->mmod->max_veloc) d->ldsk->veloc->deriv[j] = tree->mmod->max_veloc;
    }
...
{  
  ...
  if(tree->mod->s_opt->opt_clock_r == YES && tree->rates->init_clock_r == NO)
    {
...
void MCMC_Alpha(t_tree *tree)
{
  ...
  MCMC_Single_Param_Generic(&(tree->mod->ras->alpha->v),0.,100.,tree->mcmc->num_move_ras,
			    NULL,&(tree->c_lnL),
			    NULL,Wrap_Lk,tree->mcmc->move_type[tree->mcmc->num_move_ras],NO,NO,NULL,tree,NULL);
}
...
void MCMC_Covarion_Rates(t_tree *tree)
{
  ...
  if(u < .5)
	...
	{
	  min = 0.01;
	  max = tree->mod->m4mod->multipl_unscaled->v[1];
	}
      else if(class == tree->mod->m4mod->n_h-1)
	{
	  min = tree->mod->m4mod->multipl_unscaled->v[tree->mod->m4mod->n_h-2];
	  max = +100.;
	}
      else
	{
	  min = MIN(tree->mod->m4mod->multipl_unscaled->v[class-1],tree->mod->m4mod->multipl_unscaled->v[class+1]);
	  max = MAX(tree->mod->m4mod->multipl_unscaled->v[class-1],tree->mod->m4mod->multipl_unscaled->v[class+1]);
	}

      MCMC_Single_Param_Generic(&(tree->mod->m4mod->multipl_unscaled->v[class]),min,max,tree->mcmc->num_move_cov_rates+class+tree->mod->m4mod->n_h,
				NULL,&(tree->c_lnL),
				NULL,Wrap_Lk,tree->mcmc->move_type[tree->mcmc->num_move_cov_rates+class+tree->mod->m4mod->n_h],NO,NO,NULL,tree,NULL);
    }
  else
    {
      MCMC_Single_Param_Generic(&(tree->mod->m4mod->h_fq_unscaled->v[class]),0.01,+100.,tree->mcmc->num_move_cov_rates+class,
				NULL,&(tree->c_lnL),
				NULL,Wrap_Lk,tree->mcmc->move_type[tree->mcmc->num_move_cov_rates+class],NO,NO,NULL,tree,NULL);
    }
...
void MCMC_Covarion_Switch(t_tree *tree)
{
  ...
  Set_Update_Eigen(YES,tree->mod);
  MCMC_Single_Param_Generic(&(tree->mod->m4mod->delta->v),0.01,+100.,tree->mcmc->num_move_cov_switch,
			    NULL,&(tree->c_lnL),
			    NULL,Wrap_Lk,tree->mcmc->move_type[tree->mcmc->num_move_cov_switch],
                            NO,NO,NULL,tree,NULL); 
  Set_Update_Eigen(NO,tree->mod);
}
...
void MCMC_Nu(t_tree *tree)
{
  ...
  /* PhyML_Printf("\n. cur_nu: %f new_nu: %f cur_lnL_rate: %f new_lnL_rate: %f cur_lnP_rate: %f new_lnP_rate: %f ratio: %f", */
  /*              cur_nu, */
  /*              new_nu, */
  /*              cur_lnL_rate, */
  /*              new_lnL_rate, */
...
void MCMC_Complete_MCMC(t_mcmc *mcmc, t_tree *tree)
{
  ...
  mcmc->num_move_phyrex_iou_theta_sigsq       = mcmc->n_moves; mcmc->n_moves += 1;
  ...
  strcpy(mcmc->move_name[mcmc->num_move_rates_shrink],"rates_shrink");
  strcpy(mcmc->move_name[mcmc->num_move_obs_var], "observational_var");
  strcpy(mcmc->move_name[mcmc->num_move_phyrex_tip_loc], "tip_location");
  strcpy(mcmc->move_name[mcmc->num_move_phyrex_iou_theta_sigsq], "phyrex_iou_theta_sigsq");
  ...
  mcmc->move_type[mcmc->num_move_rates_shrink] = MCMC_MOVE_SCALE_THORNE;
  mcmc->move_type[mcmc->num_move_obs_var] = MCMC_MOVE_SCALE_THORNE;
  mcmc->move_type[mcmc->num_move_phyrex_tip_loc] = MCMC_MOVE_SCALE_THORNE;
  ...
  mcmc->move_prob[mcmc->num_move_updown_t_br]           = 1.0;
#if defined (INVITEE)
  ...
  mcmc->move_prob[mcmc->num_move_rates_shrink]          = 1.0;
  mcmc->move_prob[mcmc->num_move_obs_var]               = 1.0;
...
# if defined (PHYREX)
  ...
  mcmc->move_prob[mcmc->num_move_phyrex_sigsq]                 = 2.0;
  ...
  mcmc->move_prob[mcmc->num_move_phyrex_all_veloc]             = 1.0; 
  ...
  mcmc->move_prob[mcmc->num_move_phyrex_tip_loc]               = 0.0;
  mcmc->move_prob[mcmc->num_move_phyrex_iou_theta_sigsq]       = 1.0;
...
# else
  ...
  mcmc->move_prob[mcmc->num_move_rates_shrink]                 = 0.0;
  mcmc->move_prob[mcmc->num_move_obs_var]                      = 0.0;
  mcmc->move_prob[mcmc->num_move_phyrex_tip_loc]               = 0.0;
  mcmc->move_prob[mcmc->num_move_phyrex_iou_theta_sigsq]       = 0.0;
  ...
  for(i=0;i<mcmc->n_moves;i++) mcmc->start_ess[i] = YES;
  ...
  do
    {
      ...
      if(mcmc->run > mcmc->chain_len_burnin)
        for(i=0;i<mcmc->n_moves;i++) mcmc->adjust_tuning[i] = NO;
      else
        {
          for(i=0;i<mcmc->n_moves;i++) mcmc->adjust_tuning[i] = YES;
...
void MCMC_Run_Core(t_tree *tree)
{
  ...
  if(!strcmp(tree->mcmc->move_name[move],"phyrex_iou_theta_sigsq")) MCMC_PHYREX_IOU_Update_Theta_Sigsq(tree);
...
void MCMC_Read_Param_Vals(t_tree *tree)
{
  ...
  /* PhyML_Printf("\n. LnLike = %f",val); */
  fscanf(in_fp,"%lf\t",&val); // LnPriorRate
  /* PhyML_Printf("\n. LnPrior = %f",val); */
  fscanf(in_fp,"%lf\t",&val); // LnPriorTime
  /* PhyML_Printf("\n. LnPrior = %f",val); */
...
void MCMC_PHYREX_Sigsq(t_tree *tree, int print)
{
          ...
          if(tree->mcmc->run_move[tree->mcmc->num_move_phyrex_sigsq] >= 0)
            {
              ...
              aux_tree->mmod->sigsq[dim_idx] = new_sigsq;
              new_glnL_aux = LOCATION_Lk(NULL,aux_tree);

              aux_tree->mmod->sigsq[dim_idx] = cur_sigsq;
              cur_glnL_aux = LOCATION_Lk(NULL,aux_tree);
  ...
  else if((RRW_Is_Rw(tree->mmod) == YES || VELOC_Is_Integrated_Velocity(tree->mmod) == YES) && tree->mmod->sampling_scheme == SPATIAL_SAMPLING_DETECTION)
   {      
      phydbl cur_sigsq,new_sigsq;
      ...
      if(u > alpha) /* Reject */
        {
          // if(print == YES) PhyML_Printf("  reject");
          tree->mmod->sigsq[dim_idx] = cur_sigsq;
          Reset_Lk(tree);
          Reset_Prior(tree);
        }
        ...
        {
          // if(print == YES) PhyML_Printf("  accept");
          tree->mcmc->acc_move[tree->mcmc->num_move_phyrex_sigsq]++;
        }
...
void MCMC_PHYREX_Sigsq_Scale(t_tree *tree, int print)
{
  ...
  phydbl cur_lnL_geo_aux,new_lnL_geo_aux;
  phydbl cur_lnP_geo_aux,new_lnP_geo_aux;
  ...
  if(serial == NO)
    {
      ...
      sigsq_scale_bkp = (phydbl *)mCalloc(2*tree->n_otu-1,sizeof(phydbl));
      ...
      cur_lnP_geo_aux = UNLIKELY;
      new_lnP_geo_aux = UNLIKELY;
      
      cur_lnL_geo_aux = UNLIKELY;
      new_lnL_geo_aux = UNLIKELY;
      ...
      if(tree->eval_glnL == YES)
        {
          ...
          ratio += (new_lnP_geo - cur_lnP_geo);
          ratio += (cur_lnL_geo_aux - new_lnL_geo_aux);
          ratio += (cur_lnP_geo_aux - new_lnP_geo_aux);
        }
      ratio += hr;
      ratio = exp(ratio);
      alpha = MIN(1.,ratio);
      ...
      if(print == YES)
        PhyML_Printf("\n. SIGSQSCAL lnL: %12f->%12f lnP: %12f->%12f lnLaux: %12f->%12f lnPaux: %12f->%12f T: %12f hr: %12f alpha: %12f",
                     cur_lnL_geo,new_lnL_geo,
                     cur_lnP_geo,new_lnP_geo,
                     cur_lnL_geo_aux,new_lnL_geo_aux,
                     cur_lnP_geo_aux,new_lnP_geo_aux,
    ...
    {
      if(tree->mmod->sampling_scheme == SPATIAL_SAMPLING_SURVEY) assert(false);
      ...
      for(i=0;i<2*tree->n_otu-1;++i)
        {
          ...
          if(new_scale < tree->mmod->sigsq_scale_max && new_scale > tree->mmod->sigsq_scale_min) 
            {
              tree->mmod->sigsq_scale[permut[i]] = new_scale;
  ...
  if(tree->times->model_id == COALESCENT &&
     ...
     tree->mmod->sampling_scheme == SPATIAL_SAMPLING_SURVEY)
    {
      ...
      tree->mcmc->run_move[tree->mcmc->num_move_time_neff]++;
          ...
          (RRW_Is_Rw(tree->mmod) == YES || VELOC_Is_Integrated_Velocity(tree->mmod) == YES)  &&
...
void MCMC_PHYREX_Indel_Disk(t_tree *tree)
{
  ...
  if(RRW_Is_Rw(tree->mmod) == YES || VELOC_Is_Integrated_Velocity(tree->mmod) == YES) return;  
  if(tree->mmod->use_locations == NO) return;
...
void MCMC_PHYREX_Delete_Disk(phydbl hr, int n_delete_disks, phydbl cur_lbda, phydbl cur_mu, phydbl cur_rad, t_tree *tree)
{
  ...
  assert(RRW_Is_Rw(tree->mmod) == NO && VELOC_Is_Integrated_Velocity(tree->mmod) == NO);
...
void MCMC_PHYREX_Insert_Disk(phydbl hr, int n_insert_disks, phydbl cur_lbda, phydbl cur_mu, phydbl cur_rad, t_tree *tree)
{
  ...
  if(tree->eval_glnL == YES || tree->eval_tlnL == YES) PHYREX_Update_Lindisk_List(tree);
...
void MCMC_PHYREX_Move_Disk_Updown(t_tree *tree, int print)
{
  ...
  for(i=0;i<n_move_disks;i++)
    {      
      ...
      if(tree->eval_glnL == YES) new_glnL = LOCATION_Lk(NULL,tree);      
      if(tree->eval_tlnL == YES) new_tlnL = TIMES_Lk(tree);      
      ...
      if(tree->eval_tlnL == YES) ratio += (new_tlnL - cur_tlnL);      
      ratio += hr;
  ...
  if(PHYREX_Check_Struct(tree,NO) &&
     ...
     tree->rates->clock_r < tree->rates->max_clock)
    {
      ...
      */
      ...
      if(tree->eval_rlnL == YES)
        {
          new_rlnL = RATES_Lk(tree);
          new_rlnP = RATES_Prior(tree);
        }
    }
  ...
  if(print == YES)
    {
      PhyML_Printf("\n. SCALE [%6d] Scale times glnL: %10g->%10g[%2d] alnL: %10g->%10g[%2d] rlnL: %10g->%10g[%2d] tlnL: %10g->%10g[%2d] ratio: %10g scale: %10g neff=%10g T=%10g->%10g clock: %10g check: %2d",
                   ...
                   cur_tlnL,new_tlnL,tree->eval_tlnL,
...
void MCMC_PHYREX_Shuffle_Node_Times(t_tree *tree, int print)
{
  ...
  phydbl ratio,alpha,hr;
  int i;
  int move_num,err,n_nodes,idx;
  t_ldsk *ldsk_next,*n_ldsk;
  
  if(tree->mod->s_opt->opt_node_ages == NO) return;
  if(tree->eval_tlnL == NO) return;

  n_nodes = 1+(int)(0.1*tree->n_otu);

  TIMES_Record_Times(tree);

  move_num = tree->mcmc->num_move_phyrex_shuffle_node_times;
  ratio    = 0.0;
  ...
  do
    {
      idx = Rand_Int(tree->n_otu,2*tree->n_otu-3);

      assert(tree->a_nodes[idx] != tree->n_root);
      assert(tree->a_nodes[idx] != NULL);
      assert(tree->a_nodes[idx]->tax == NO);

      n_ldsk = tree->a_nodes[idx]->ldsk;
      
      assert(n_ldsk->next != NULL);
      assert(n_ldsk->n_next == 2);
      assert(n_ldsk->prev != NULL);

      cur_t = n_ldsk->disk->time;
      
      ldsk_next = NULL;  
      t_min = tree->times->nd_t[n_ldsk->prev->nd->num];
      t_max = +INFINITY;
      for(i=0;i<n_ldsk->n_next;++i)
        if(n_ldsk->next[i]->disk->time < t_max)
          {
            t_max = n_ldsk->next[i]->disk->time;
            ldsk_next = n_ldsk->next[i];
          }
      ...
      PHYREX_Move_Disk_Updown(n_ldsk->disk,new_t,tree);
      n_ldsk->disk->time = new_t;
      tree->times->nd_t[tree->a_nodes[idx]->num] = new_t; // Ok only if tree is binary
      
      PHYREX_Update_Lindisk_List_Range(ldsk_next->disk,n_ldsk->prev->disk,tree);
    }
  while(--n_nodes > 0);
  ...
  if(print == YES)
    PhyML_Printf("\n. PHYREX_Shuffle_Node_Time %6d alnL:%f->%f glnL:%f->%f rlnL:%f->%f tlnL:%f->%f alpha:%f",
                 tree->mcmc->run,
                 cur_alnL,new_alnL,
                 cur_glnL,new_glnL,
                 cur_rlnL,new_rlnL,
  ...
  if(u > alpha) /* Reject */
    {
      ...
      for(i=tree->n_otu;i<2*tree->n_otu-1;++i)
        {          
          PHYREX_Move_Disk_Updown(tree->a_nodes[i]->ldsk->disk,tree->times->nd_t[i],tree);
          tree->a_nodes[i]->ldsk->disk->time = tree->times->nd_t[i];
        }
...
void MCMC_PHYREX_Prune_Regraft(t_tree *tree, int print)
{
  ...
  while(n_iter--)
    {      
      ...
      if(tree->eval_tlnL == YES) assert(cur_tlnL > UNLIKELY);
  ...
  while(n_iter--)
    {
      ...
      /*   phydbl delta2 = LOCATION_Lk(NULL,tree) - cur_glnL; */
      /*   /\* PhyML_Printf("\n<<>> delta1=%f delta2=%f",delta1,delta2); *\/ */
      /*   if(Are_Equal(delta1,delta2,1.E-5)==NO) { */
      /*     { */
      /*       PhyML_Printf("\n<> delta1=%f delta2=%f",delta1,delta2); */
      ...
      if(u > alpha) /* Reject */
        ...
        {
          /* PhyML_Printf("\n. Accept --> %f %f",LOCATION_Lk(NULL,tree),new_glnL); */
          tree->mmod->c_lnL = new_glnL;
          tree->times->c_lnL = new_tlnL;
...
//   K = tree->mcmc->tune_move[tree->mcmc->num_move_phyrex_ldsk_given_disk];
  ...
  for(j=0;j<tree->mmod->n_dim;++j) rad[j] = 1.E-2; 
      ...
      for(j=0;j<tree->mmod->n_dim;j++)
        {
          ...
          else if(RRW_Is_Rw(tree->mmod) == YES || VELOC_Is_Integrated_Velocity(tree->mmod) == YES)
            {
      ...
      if(type < pindel) /* Insert */
        {
          ...
          new_glnL = cur_glnL;
          new_tlnL = cur_tlnL;
          if(tree->eval_glnL == YES) new_glnL -= LOCATION_Lk_Range(youngest_disk,oldest_disk,tree);
          if(tree->eval_tlnL == YES) new_tlnL -= TIMES_Lk_Range(youngest_disk,oldest_disk,tree);
      ...
      if(type < pindel) /* Insert */
        {
          ...
          new_glnL = cur_glnL;
          new_tlnL = cur_tlnL;
          if(tree->eval_glnL == YES) new_glnL -= LOCATION_Lk_Range(young_disk,old_disk,tree);
          if(tree->eval_tlnL == YES) new_tlnL -= TIMES_Lk_Range(young_disk,old_disk,tree);
      ...
      else /* Remove disk */
        {
          ...
          new_glnL = cur_glnL;
          new_tlnL = cur_tlnL;
          if(tree->eval_glnL == YES) new_glnL -= LOCATION_Lk_Range(target_disk->next,target_disk->prev,tree);
          if(tree->eval_tlnL == YES) new_tlnL -= TIMES_Lk_Range(target_disk->next,target_disk->prev,tree);
...
void MCMC_PHYREX_Wide_Exchange(t_tree *tree, int print)
{
  ...
  while(n_iter--)
    {      
      ...
      /*   phydbl delta2 = LOCATION_Lk(NULL,tree) - cur_glnL; */
      /*   /\* PhyML_Printf("\n<<>> delta1=%f delta2=%f",delta1,delta2); *\/ */
      /*   if(Are_Equal(delta1,delta2,1.E-8)==NO) { */
      /*     { */
      /*       PhyML_Printf("\n<> delta1=%f delta2=%f",delta1,delta2); */
      /*       assert(FALSE); */
      /*     } */
      ...
      /* PhyML_Printf("\n. LOCATION_Lk: %f",LOCATION_Lk(NULL,tree)); */
      ...
      if(u > alpha) /* Reject */
        {
          ...
          /* if(tree->eval_glnL == YES && (IBM_Is_Ibm(tree->mmod) == YES)) PHYREX_Restore_Coord(tree); */
...
#ifdef PHYREX
void MCMC_PHYREX_Node_Velocity(t_tree *tree)
{
  if(tree->mmod->use_locations == NO) return;
  ...
  tree->contmod->both_sides[VELOCITY] = YES;
  LOCATION_Lk(NULL,tree);
  MCMC_PHYREX_Node_Velocity_Pre(NULL,tree->n_root,tree);
  tree->contmod->both_sides[LOCATION] = NO;
  tree->contmod->both_sides[VELOCITY] = NO;
  LOCATION_Lk(NULL,tree);
...
//   for (i = 0; i < 2 * tree->n_otu - 1; ++i)
//   {
//       tree->mcmc->run_move[tree->mcmc->num_move_phyrex_node_veloc]++;
...
//       PHYREX_Store_Veloc(n->ldsk->veloc);
...
//           /* new = Rnorm(cur,sqrt(fabs(mean_v[j]/2.))); */
...
//           new = Rnorm(cur, 1.E-1);
//           hr -= Log_Dnorm(new, cur, 1.E-1, &err);
//           hr += Log_Dnorm(cur, new, 1.E-1, &err);
...
//       if (u > alpha) /* Reject */
//       {
//           PHYREX_Restore_Veloc(n->ldsk->veloc);
//           tree->mmod->c_lnL = cur_glnL;
//       }
//       else
//       {
//           tree->mmod->c_lnL = new_glnL;
//           tree->mcmc->acc_move[tree->mcmc->num_move_phyrex_node_veloc]++;
//       }
...
void MCMC_PHYREX_Node_Velocity_Pre(t_node *a, t_node *d, t_tree *tree)
{
    phydbl cur_glnL, new_glnL;
    ...
    PHYREX_Store_Veloc(d->ldsk->veloc);
    ...
    if (dad != tree->n_root)
        dt_dad = tree->times->nd_t[dad->num] - tree->times->nd_t[a->num];
    else
        dt_dad = -1.;
    ...
    for (i = 0; i < tree->mmod->n_dim; ++i)
    {
        cur_vel = d->ldsk->veloc->deriv[i];
        
        var = mu = var_dad = mu_dad = var_son = mu_son = var_bro = mu_bro = -1.;

        // new_vel = Rnorm(cur_vel,1.E-3);
        // hr -= Log_Dnorm(new_vel,cur_vel,1.E-3,&err);
        // hr += Log_Dnorm(cur_vel,new_vel,1.E-3,&err);

        // if (dad != tree->n_root)
        // {
        //   new_vel = Rnorm(cur_vel, VELOC_Velocity_Variance_Along_Edge(dad, i, tree));
        //   hr -= Log_Dnorm(new_vel, cur_vel, VELOC_Velocity_Variance_Along_Edge(dad, i, tree), &err);
        //   hr += Log_Dnorm(cur_vel, new_vel, VELOC_Velocity_Variance_Along_Edge(dad, i, tree), &err);
        // }
        // else
        // {
        //   new_vel = Rnorm(cur_vel, tree->mmod->rw_root_var[VELOCITY]);
        //   hr -= Log_Dnorm(new_vel, cur_vel, tree->mmod->rw_root_var[VELOCITY], &err);
        //   hr += Log_Dnorm(cur_vel, new_vel, tree->mmod->rw_root_var[VELOCITY], &err);
        // }

        // new_vel = Rnorm(cur_vel,.05*cur_vel);
        // hr -= Log_Dnorm(new_vel,cur_vel,.05*cur_vel,&err);
        // hr += Log_Dnorm(cur_vel,new_vel,.05*new_vel,&err);
        ...
        if (dad != tree->n_root)
        {
            var_dad = dt_dad * tree->mmod->sigsq[i];
            mu_dad = dad->anc->ldsk->veloc->deriv[i];
        }
        else
        {
            var_dad = tree->mmod->rw_root_var[VELOCITY];
            mu_dad = dad->ldsk->veloc->deriv[i];
        }
        ...
        {
            var_son = dt_son * tree->mmod->sigsq[i];
            var_bro = dt_bro * tree->mmod->sigsq[i];

            mu_son = son->ldsk->veloc->deriv[i];
            mu_bro = bro->ldsk->veloc->deriv[i];

            if (var_dad > SMALL && var_son > SMALL && var_bro > SMALL)
            {
                var = 1. / (1. / var_dad + 1. / var_son + 1. / var_bro);
                mu = var * (mu_dad / var_dad + mu_son / var_son + mu_bro / var_bro);
            }
            else if (var_dad < SMALL)
            {
                var = 0.0;
                mu = mu_dad;
            }
            else if (var_son < SMALL)
            {
                var = 0.0;
                mu = mu_son;
            }
            else if (var_bro < SMALL)
            {
                var = 0.0;
                mu = mu_bro;
            }
    ...
    if (u > alpha) /* Reject */
    {
        PHYREX_Restore_Veloc(d->ldsk->veloc);
        tree->mmod->c_lnL = cur_glnL;
    ...
    tree->mcmc->run++;
    tree->mmod->do_location_sampling = NO;
    PHYREX_Print_MCMC_Stats(tree);
    PHYREX_Print_MCMC_Tree(tree);
    PHYREX_Print_MCMC_Summary(tree);
    tree->mmod->do_location_sampling = YES;
    ...
    else
    {
        for (i = 0; i < 3; ++i)
            if (d->v[i] != a && !(a == tree->n_root && d->b[i] == tree->e_root))
            {
                VELOC_Update_Lk_Velocity_Up(d, d->v[i], tree);
                VELOC_Update_Lk_Location_Up(d, d->v[i], tree);
      ...
      if(a == NULL)
        {
          ...
          new_vel[i] = Rnorm(cur_vel[i],sqrt(tree->mmod->rw_root_var[VELOCITY]));
          hr -= Log_Dnorm(new_vel[i],cur_vel[i],sqrt(tree->mmod->rw_root_var[VELOCITY]),&err);
          hr += Log_Dnorm(cur_vel[i],new_vel[i],sqrt(tree->mmod->rw_root_var[VELOCITY]),&err);
        }
      else
        {
          var = VELOC_Velocity_Variance_Along_Edge(dad,i,tree);
          cur_vel[i] = dad->ldsk->veloc->deriv[i];
          if(isnan(dad->anc->ldsk->veloc->deriv[i])) assert(false);
          if(isnan(var)) assert(false);          
...
#ifdef PHYREX
void MCMC_PHYREX_All_Velocities(t_tree *tree)
{
  if (tree->mmod->use_locations == NO)
    return;

  if (VELOC_Is_Integrated_Velocity(tree->mmod) == NO)
    return;

  int i, j, n_nodes;
  phydbl u, alpha, ratio, mult;
  phydbl cur_glnL, new_glnL;
  phydbl cur_glnP, new_glnP;
  t_node *n;
  phydbl cur_sigsq, new_sigsq;
  phydbl cur_iou_theta, new_iou_theta;
  ...
  for (i = 0; i < tree->n_otu - 1; ++i)
  {
    for (j = 0; j < tree->mmod->n_dim; ++j)
    {
      tree->mcmc->run_move[tree->mcmc->num_move_phyrex_all_veloc]++;
      ...
      cur_sigsq = tree->mmod->sigsq[j];
      new_sigsq = tree->mmod->sigsq[j];

      cur_iou_theta = tree->mmod->ou_theta;
      new_iou_theta = tree->mmod->ou_theta;
      ...
      {
          new_iou_theta = cur_iou_theta * mult;
          tree->mmod->ou_theta = new_iou_theta;
      }

      new_sigsq = cur_sigsq * mult;
      tree->mmod->sigsq[j] = new_sigsq;
      ...
      ratio += n_nodes * log(mult);
      ratio += log(mult);
      if(IOU_Is_Iou(tree->mmod) == YES) ratio += log(mult);

      ratio = exp(ratio);
      alpha = MIN(1., ratio);

      // PhyML_Printf("\n. lnL: %f->%f scale: %f alpha: %f",cur_glnL,new_glnL,mult,alpha);
      ...
      {
        PHYREX_Restore_All_Veloc(tree);
        tree->mmod->sigsq[j] = cur_sigsq;
        tree->mmod->ou_theta = cur_iou_theta;
        Reset_Lk(tree);
        Reset_Prior(tree);
      }
      else
      {
        tree->mcmc->acc_move[tree->mcmc->num_move_phyrex_all_veloc]++;
      }
...
{
  if(VELOC_Is_Integrated_Velocity(tree->mmod) == YES)
    MCMC_Single_Param_Generic(&(tree->mmod->omega),
                              tree->mmod->min_omega,
                              tree->mmod->max_omega,
...
{
    if (VELOC_Is_Integrated_Velocity(tree->mmod) == YES && IOU_Is_Iou(tree->mmod) == YES)
    {
        ...
        for (j = 0; j < tree->mmod->n_dim; ++j)
        {
            tree->mcmc->run_move[tree->mcmc->num_move_phyrex_iou_theta]++;
            ...
            cur_theta = tree->mmod->ou_theta;
            new_theta = tree->mmod->ou_theta;

            u = Uni();
            mult = exp((u - 0.5));

            new_theta = cur_theta * mult;
            tree->mmod->ou_theta = new_theta;
            ...
            // PhyML_Printf("\n. lnL: %13f->%13f scale: %13f alpha: %13f theta: %13g->%13g [%13g %13g %13g %13g]",
            //              cur_glnL, new_glnL,
            //              mult, alpha,
            //              cur_theta, new_theta,
            //              tree->contmod->lnL[VELOCITY*tree->mmod->n_dim+0],
            ...
            if (u > alpha) /* Reject */
            {
                tree->mmod->ou_theta = cur_theta;
                Reset_Lk(tree);
                Reset_Prior(tree);
            }
            else
            {
                tree->mcmc->acc_move[tree->mcmc->num_move_phyrex_iou_theta]++;
            }
...
void MCMC_PHYREX_IOU_Update_Theta_Sigsq(t_tree *tree)
{
    if (VELOC_Is_Integrated_Velocity(tree->mmod) == YES && IOU_Is_Iou(tree->mmod) == YES)
    {
        ...
        for (j = 0; j < tree->mmod->n_dim; ++j)
        {
            tree->mcmc->run_move[tree->mcmc->num_move_phyrex_iou_theta_sigsq]++;
            ...
            {
                tree->mcmc->acc_move[tree->mcmc->num_move_phyrex_iou_theta_sigsq]++;
            }
...
void MCMC_PHYREX_Tip_Location(int tip_num, t_tree *tree)
{
  ...
  for(int j=0;j<tree->mmod->n_dim;++j)
    MCMC_Single_Param_Generic(&(tree->a_nodes[tip_num]->ldsk->coord->lonlat[j]),
                              tree->mmod->lim_do->lonlat[j],
                              tree->mmod->lim_up->lonlat[j],
                              tree->mcmc->num_move_phyrex_tip_loc,
                              NULL,&(tree->mmod->c_lnL),
                              NULL,LOCATION_Wrap_Lk,
                              tree->mcmc->move_type[tree->mcmc->num_move_phyrex_tip_loc],
                              NO,print,NULL,tree,NULL);
}
#endif
...
void MCMC_Crossvalidate_Locations(t_tree *tree)
{
  ...
  for(i=0;i<tree->n_otu;++i)
    {
      PhyML_Printf("\n. Posterior prediction of location for tip '%s'...",tree->a_nodes[i]->name);
      
      // Store location
      PHYREX_Store_Geo_Coord(tree->a_nodes[i]->ldsk->coord);

      LOCATION_Lk(NULL,tree);
      PhyML_Printf("\n. Location lk: %f",tree->mmod->c_lnL);
      
      // Run shortish MCMC, printing error in predicted location at every step
      mcmc->run = 0;
      mcmc->chain_len = 1E+6;
      mcmc->sample_interval = -1; // no sampling of parameters
      mcmc->print_every = -1;
      mcmc->chain_len_burnin = 0;
      mcmc->is_burnin = NO;
      mcmc->tune_move[mcmc->num_move_phyrex_tip_loc] = 1.0;
      do
        {
          MCMC_Run_Core(tree);
          
          MCMC_PHYREX_Tip_Location(i,tree);
          
          mse = 0.0;
          /* for(j=0;j<tree->mmod->n_dim;++j) */
          /*   mse += */
          /*     POW(tree->a_nodes[i]->ldsk->coord->lonlat[j]- */
          /*         tree->a_nodes[i]->ldsk->coord->cpy->lonlat[j],2); */
          mse += Haversine_Distance(tree->a_nodes[i]->ldsk->coord,tree->a_nodes[i]->ldsk->coord->cpy);

          if(!(mcmc->run%50))
            PhyML_Printf("\nCV %20s %15f %15f %15f %15f %15f %15f",
                         tree->a_nodes[i]->name,
                         mse,
                         tree->a_nodes[i]->ldsk->coord->lonlat[0],
                         tree->a_nodes[i]->ldsk->coord->lonlat[1],
                         tree->a_nodes[i]->ldsk->coord->cpy->lonlat[0],
                         tree->a_nodes[i]->ldsk->coord->cpy->lonlat[1],
                         LOCATION_Lk(NULL,tree));
