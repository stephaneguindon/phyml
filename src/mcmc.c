...
# if defined (PHYREX)
  mcmc->move_weight[mcmc->num_move_phyrex_lbda]                  = 4.0;
  mcmc->move_weight[mcmc->num_move_phyrex_mu]                    = 6.0;
  mcmc->move_weight[mcmc->num_move_phyrex_rad]                   = 4.0;
  ...
  For(j,n_iter)
    {
      ...
      hr += PHYREX_Path_Logdensity(start_ldsk,end_ldsk,rate*dt,tree->mmod->rad,tree);
      
      new_path = PHYREX_Generate_Path(start_ldsk,end_ldsk,rate*dt,tree->mmod->rad,tree);
