...
void MCMC_MIGREP_Indel_Disk(t_tree *tree)
{
  ...
  /* max = n_disks_cur + 10.; */
  /* min = MAX(0.0,n_disks_cur - 10.); */
  /* n_disks_new = (int)(Uni()*(max-min) + min); */
  /* hr += LOG(max - min); */

  /* max = n_disks_new + 10.; */
  /* min = MAX(0.0,n_disks_new - 10.); */
  /* hr -= LOG(max - min); */


  /* phydbl u = Uni(); */
  /* if(u < 0.5) n_disks_new = n_disks_cur + 1; */
  /* else n_disks_new = n_disks_cur - 1; */

  /* if(n_disks_new <= 0) return; */
...
{
  phydbl u,alpha,ratio;
  phydbl cur_glnL, new_glnL, T, cur_lbda, new_lbda, cur_mu;
  ...
  /* min_mu  = tree->mmod->mu; */
  /* max_mu  = MIN(tree->mmod->mu + 0.1,tree->mmod->max_mu); */
  /* hr     -= LOG(1./(max_mu-min_mu)); */
 
  /* tree->mmod->mu = Uni()*(max_mu - min_mu) + min_mu; */

  /* min_mu = MAX(tree->mmod->min_mu,tree->mmod->mu - 0.1); */
  /* max_mu = tree->mmod->mu; */
  /* hr    += LOG(1./(max_mu-min_mu)); */  
      ...
      if(target_disk[i]->ldsk != NULL)
        {
          ...
          /*   { */
          /*     max = MIN(tree->mmod->lim->lonlat[j],target_disk[i]->centr->lonlat[j] + 0.1); */
          /*     min = MAX(0.0,target_disk[i]->centr->lonlat[j] - 0.1); */
          /*     hr  += LOG(max-min); */
  ...
  For(i,n_move_ldsk)
    {
      ...
      if(tree->mmod->name == MIGREP_UNIFORM)
      ...
      else if(tree->mmod->name == MIGREP_NORMAL)
        {
          ...
          For(j,tree->mmod->n_dim)
            target_disk[i]->ldsk->coord->lonlat[j] =
            Rnorm_Trunc(target_disk[i]->centr->lonlat[j],tree->mmod->rad,0.0,tree->mmod->lim->lonlat[j],&err);
          ...
          /* For(j,tree->mmod->n_dim) */
          /*   { */
          /*     max = MIN(tree->mmod->lim->lonlat[j],target_disk[i]->ldsk->coord->lonlat[j] + 0.1); */
          /*     min = MAX(0.0,target_disk[i]->ldsk->coord->lonlat[j] - 0.1); */
          /*     hr  += LOG(max-min); */
...
void MCMC_MIGREP_Indel_Hit(t_tree *tree)
{
  ...
  /* max = n_disks_cur + 1; */
  /* min = MAX(0.0,n_disks_cur - 1.); */
  /* n_disks_new = (int)(Uni()*(max-min) + min); */
  /* hr += LOG(max - min); */

  /* max = n_disks_new + 1.; */
  /* min = MAX(0.0,n_disks_new - 1.); */
  /* hr -= LOG(max - min); */
  ...
  For(j,n_insert_disks)
    {  
      ...
      For(i,tree->mmod->n_dim)
        {
          new_disk[j]->centr->lonlat[i] = Rnorm_Trunc(0.5*(young_ldsk[j]->coord->lonlat[i]+old_ldsk[j]->coord->lonlat[i]),
                                                      1.0*tree->mmod->rad,
...
void MCMC_MIGREP_Prune_Regraft(t_tree *tree)
{
  ...
  regraft_next_num = MIGREP_Random_Insert_Ldsk_In_Next_List(prune_daughter_ldsk,regraft_ldsk);
