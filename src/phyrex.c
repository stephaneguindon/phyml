...
void PHYREX_XML(char *xml_filename)
{
      ...
      if(modname != NULL)
        {
          ...
          else if(!strcmp(xnd->attr->value,"bmp")) mixt_tree->mmod->id = BMP;
          else assert(FALSE);
        }
    }
  ...
  mixt_tree->mmod->rad  = mixt_tree->mmod->max_rad;
  ...
  if(mixt_tree->mmod->id == BMP)
    {
      PHYREX_Print_Struct('*',mixt_tree);
