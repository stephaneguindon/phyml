...
void MIXT_Chain_All(t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    MIXT_Chain_String(curr->mod->custom_mod_string,
                      next->mod->custom_mod_string);
    ...
    MIXT_Chain_Scalar_Dbl(curr->mod->br_len_mult_unscaled,
                          next->mod->br_len_mult_unscaled);
    ...
    for (i = 0; i < 2 * mixt_tree->n_otu - 1; ++i)
      MIXT_Chain_Scalar_Dbl(curr->a_edges[i]->l_old, next->a_edges[i]->l_old);
    for (i = 0; i < 2 * mixt_tree->n_otu - 1; ++i)
      MIXT_Chain_Scalar_Dbl(curr->a_edges[i]->l_var, next->a_edges[i]->l_var);
    for (i = 0; i < 2 * mixt_tree->n_otu - 1; ++i)
      MIXT_Chain_Scalar_Dbl(curr->a_edges[i]->l_var_old,
                            next->a_edges[i]->l_var_old);
...
void MIXT_Chain_Sprs(t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    {
      if (tree->next)
        tree->spr_list_one_edge[i]->next = tree->next->spr_list_one_edge[i];
      if (tree->prev)
        tree->spr_list_one_edge[i]->prev = tree->prev->spr_list_one_edge[i];
      if (tree->next_mixt)
        tree->spr_list_one_edge[i]->next_mixt =
            tree->next_mixt->spr_list_one_edge[i];
      if (tree->prev_mixt)
        tree->spr_list_one_edge[i]->prev_mixt =
            tree->prev_mixt->spr_list_one_edge[i];

      if (tree->next)
        tree->spr_list_all_edge[i]->next = tree->next->spr_list_all_edge[i];
      if (tree->prev)
        tree->spr_list_all_edge[i]->prev = tree->prev->spr_list_all_edge[i];
      if (tree->next_mixt)
        tree->spr_list_all_edge[i]->next_mixt =
            tree->next_mixt->spr_list_all_edge[i];
      if (tree->prev_mixt)
        tree->spr_list_all_edge[i]->prev_mixt =
            tree->prev_mixt->spr_list_all_edge[i];
    }
    tree = tree->next;
  } while (tree);
}
...
phydbl MIXT_Lk(t_edge *mixt_b, t_tree *mixt_tree)
{
  ...
  if (mixt_b == NULL)
  {
    ...
    do
    {
      ...
      if (!cpy_mixt_b)
      {
        if (!Update_Boundaries(tree->mod) || !Update_Efrq(tree->mod) ||
            !Update_Eigen(tree->mod))
        {
          PhyML_Fprintf(stderr, "\n. move: %s",
                        mixt_tree->mcmc->move_name[mixt_tree->mcmc->move_idx]);
  ...
  if (!cpy_mixt_b)
  {
    for (br = 0; br < 2 * mixt_tree->n_otu - 3; ++br)
      MIXT_Update_PMat_At_Given_Edge(mixt_tree->a_edges[br], mixt_tree);
  ...
  else
  {
    if (mixt_tree->n_root != NULL &&
        (mixt_b == mixt_tree->n_root->b[1] ||
         mixt_b == mixt_tree->n_root->b[2]) &&
  ...
  if (!cpy_mixt_b)
  {
    if (mixt_tree->n_root != NULL)
    {
      ...
      {
        MIXT_Post_Order_Lk(mixt_tree->n_root, mixt_tree->n_root->v[1],
                           mixt_tree);
        MIXT_Post_Order_Lk(mixt_tree->n_root, mixt_tree->n_root->v[2],
                           mixt_tree);

        MIXT_Update_Partial_Lk(mixt_tree, mixt_tree->n_root->b[1],
                               mixt_tree->n_root);
        MIXT_Update_Partial_Lk(mixt_tree, mixt_tree->n_root->b[2],
                               mixt_tree->n_root);

        if (mixt_tree->both_sides == YES)
        {
          MIXT_Pre_Order_Lk(mixt_tree->n_root, mixt_tree->n_root->v[1],
                            mixt_tree);
          MIXT_Pre_Order_Lk(mixt_tree->n_root, mixt_tree->n_root->v[2],
                            mixt_tree);
        }
      }
      else
      {
        MIXT_Post_Order_Lk(mixt_tree->e_root->rght, mixt_tree->e_root->left,
                           mixt_tree);
        MIXT_Post_Order_Lk(mixt_tree->e_root->left, mixt_tree->e_root->rght,
                           mixt_tree);

        if (mixt_tree->both_sides == YES)
        {
          MIXT_Pre_Order_Lk(mixt_tree->e_root->rght, mixt_tree->e_root->left,
                            mixt_tree);
          MIXT_Pre_Order_Lk(mixt_tree->e_root->left, mixt_tree->e_root->rght,
                            mixt_tree);
        }
      }
    }
    ...
    {
      MIXT_Post_Order_Lk(mixt_tree->a_nodes[0], mixt_tree->a_nodes[0]->v[0],
                         mixt_tree);
      if (mixt_tree->both_sides == YES)
      {
        MIXT_Pre_Order_Lk(mixt_tree->a_nodes[0], mixt_tree->a_nodes[0]->v[0],
                          mixt_tree);
      }
    }
  }
  ...
  do /*! Consider each element of the data partition */
  {
    ...
    if (!cpy_mixt_b)
    {
      ...
      {
        if (mixt_tree->ignore_root == NO)
          mixt_b = (mixt_tree->n_root->v[1]->tax == NO)
                       ? (mixt_tree->n_root->b[2])
                       : (mixt_tree->n_root->b[1]);
        else
    ...
    sum_scale_left_cat = (phydbl *)mCalloc(
        MAX(mixt_tree->mod->ras->n_catg, mixt_tree->mod->n_mixt_classes),
        sizeof(phydbl));
    sum_scale_rght_cat = (phydbl *)mCalloc(
        MAX(mixt_tree->mod->ras->n_catg, mixt_tree->mod->n_mixt_classes),
        sizeof(phydbl));
    ...
    sum_probas = MIXT_Get_Sum_Of_Probas_Across_Mixtures(
        r_mat_weight_sum, e_frq_weight_sum, mixt_tree);
    ...
    do
    {
      ...
      if (tree->use_eigen_lr == YES)
      {
        ...
        len *= tree->mixt_tree->mod->ras->gamma_rr
        ...
        for (l = 0; l < tree->mod->ns; l++)
          expl[l] = (phydbl)POW(tree->mod->eigen->e_val[l], len);
      }
    ...
    for (site = 0; site < mixt_tree->data->n_pattern; ++site)
    {
      b    = mixt_b->next;
      tree = mixt_tree->next;
      ...
      if ((b->rght->tax) && (!mixt_tree->mod->s_opt->greedy) &&
      ...
      do
      {
            ...
            (tree->data->wght[tree->curr_site] > SMALL))
        {
          ...
          else
          {
            if (b->rght->tax == YES)
              site_lk_cat = Lk_Core(
                  state, ambiguity_check, b->p_lk_left + site * ns,
                  b->p_lk_tip_r + site * ns, b->Pij_rr, b->tPij_rr, b, tree);
            else
              site_lk_cat = Lk_Core(
                  state, ambiguity_check, b->p_lk_left + site * ns,
                  b->p_lk_rght + site * ns, b->Pij_rr, b->tPij_rr, b, tree);
          }
        }
        ...
        sum_scale_left_cat[tree->mod->ras->parent_class_number] =
            (b->sum_scale_left) ? (b->sum_scale_left[site]) : (0.0);

        sum_scale_rght_cat[tree->mod->ras->parent_class_number] =
            (b->sum_scale_rght) ? (b->sum_scale_rght[site]) : (0.0);

        sum = sum_scale_left_cat[tree->mod->ras->parent_class_number] +
              sum_scale_rght_cat[tree->mod->ras->parent_class_number];

        if (sum > 1024.)
        {
          /* PhyML_Fprintf(stderr,"\n. Numerical precision issue detected (sum =
           * %g)!!!",sum); */
          sum                                = 1023.;
      ...
      if (mixt_tree->mod->ras->invar == YES)
      {
        ...
        while (tree->mod->ras->invar == NO)
        {
          ...
          {
            PhyML_Fprintf(stderr, "\n. tree: %p", tree);
            PhyML_Fprintf(stderr, "\n. Err in file %s at line %d", __FILE__,
                          __LINE__);
            Exit("\n");
          }
        }

        /*! 'tree' will give the correct state frequencies (as opposed to
         * mixt_tree) */
        ...
        {
          site_lk = site_lk * (1. - mixt_tree->mod->ras->pinvar->v) +
                    inv_site_lk * mixt_tree->mod->ras->pinvar->v;
        }
      }
      ...
      if (site_lk < SMALL)
      {
        ...
        /* PhyML_Fprintf(stderr,"\n. lk = %G log(lk) = %f <
         * %G",site_lk,log_site_lk,-BIG); */
        /* for(class=0;class<mixt_tree->mod->ras->n_catg;class++)
         * PhyML_Fprintf(stderr,"\n. rr=%f
         * p=%f",mixt_tree->mod->ras->gamma_rr->v[class],mixt_tree->mod->ras->gamma_r_proba->v[class]);
         */
        /* PhyML_Fprintf(stderr,"\n. pinv = %G",mixt_tree->mod->ras->pinvar->v);
         */
        /* PhyML_Fprintf(stderr,"\n. bl mult =
         * %G",mixt_tree->mod->br_len_mult->v); */
        /* PhyML_Fprintf(stderr,"\n. Err. in file %s at line
         * %d.\n",__FILE__,__LINE__); */
        /* Exit("\n"); */
  ...
  if (mixt_tree->is_mixt_tree == YES)
  {
    ...
    do
    {
      if (!class)
        ...
        n_catg = (int *)realloc(n_catg, (class + 1) * sizeof(int));
...
t_tree **MIXT_Record_All_Mixtures(t_tree *mixt_tree)
{
  ...
  {
    if (!tree_list)
      tree_list = (t_tree **)mCalloc(1, sizeof(t_tree *));
    else
      tree_list =
          (t_tree **)realloc(tree_list, (n_trees + 1) * sizeof(t_tree *));
  ...
  do
  {
        ...
        tree->next->is_mixt_tree == NO)
    {
      if (tree->mixt_tree->next_mixt == NULL)
      {
        ...
        for (i = 0; i < 2 * tree->n_otu - 2; ++i)
          tree->spr_list_one_edge[i]->next = NULL;
      }
      else
      {
        tree->next = tree->mixt_tree->next_mixt;
        for (i = 0; i < 2 * tree->n_otu - 1; ++i)
          tree->a_edges[i]->next = tree->mixt_tree->next_mixt->a_edges[i];
        for (i = 0; i < 2 * tree->n_otu - 1; ++i)
          tree->a_nodes[i]->next = tree->mixt_tree->next_mixt->a_nodes[i];
        for (i = 0; i < 2 * tree->n_otu - 2; ++i)
          tree->spr_list_one_edge[i]->next =
              tree->mixt_tree->next_mixt->spr_list_one_edge[i];
      }
    }
...
int *MIXT_Record_Has_Invariants(t_tree *mixt_tree)
{
  ...
  {
    if (!n_trees)
      has_invariants = (int *)mCalloc(1, sizeof(int));
    else
      has_invariants =
          (int *)realloc(has_invariants, (n_trees + 1) * sizeof(int));
...
void MIXT_Check_Invar_Struct_In_Each_Partition_Elem(t_tree *mixt_tree)
{
  ...
  else
  {
    ...
    do
    {
      ...
      {
        PhyML_Fprintf(
            stderr, "\n. Found %d classes of the mixture for file '%s' set to",
            n_inv, tree->mixt_tree->io->in_align_file);
        PhyML_Fprintf(
            stderr,
      ...
      {
        PhyML_Fprintf(stderr,
                      "\n. Unexpected settings for 'siterates' in a partition "
                      "element (file '%s')",
...
void MIXT_Check_RAS_Struct_In_Each_Partition_Elem(t_tree *mixt_tree)
{
  ...
  else
  {
    ...
    do
    {
      ...
      if ((tree->next && tree->next->is_mixt_tree == YES) ||
          (!tree->next)) /*! current tree is the last element of this mixture */
      {
...
void MIXT_Prune_Subtree(t_node *mixt_a, t_node *mixt_d, t_edge **mixt_target,
                        t_edge **mixt_residual, t_tree *mixt_tree)
{
  t_node *a, *d;
...
void MIXT_Graft_Subtree(t_edge *mixt_target, t_node *mixt_link,
                        t_node *mixt_link_daughter, t_edge *mixt_residual,
                        t_node *mixt_target_nd, t_tree *mixt_tree)
{
  t_edge *target, *residual;
...
phydbl MIXT_Rescale_Free_Rate_Tree(t_tree *mixt_tree)
{
  ...
  for (i = 0; i < 2 * mixt_tree->n_otu - 1; ++i)
  {
    ...
    at_boundary = NO;
    if (b->l->v > mixt_tree->mod->l_max - 1.E-100 &&
        b->l->v < mixt_tree->mod->l_max + 1.E-100)
      at_boundary = YES;
    if (b->l->v > mixt_tree->mod->l_min - 1.E-100 &&
...
phydbl MIXT_Get_Mean_Edge_Len(t_edge *mixt_b, t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    sum += b->l->v *
           (tree->mixt_tree ? tree->mixt_tree->mod->br_len_mult->v : 1.0);
    n++;
    b    = b->next;
    tree = tree->next;
  } while (b);
...
phydbl MIXT_Get_Sum_Of_Probas_Across_Mixtures(phydbl  r_mat_weight_sum,
                                              ...
                                              t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    sum += mixt_tree->mod->ras->gamma_r_proba
               ->v[tree->mod->ras->parent_class_number] *
           tree->mod->r_mat_weight->v / r_mat_weight_sum *
...
void MIXT_RATES_Update_One_Edge_Length(t_edge *mixt_b, t_tree *mixt_tree)
{
  ...
  b = mixt_tree->next->a_edges[mixt_b->num];
  ...
  do
  {
    ...
    tree = tree->next;
    if (tree != NULL) b = tree->a_edges[mixt_b->num];
  } while (tree);
}
      ...
      if (mod_in->io->cdata != mod_out->io->cdata)
      {
        if (mod_in->e_frq == mod_out->e_frq)
        {
              ...
              mod_in->whichmodel != JC69 && mod_in->whichmodel != K80)
          {
            ...
            PhyML_Fprintf(stderr, "\n. to one data set only. If you are using "
                                  "the XML interface, ");
...
t_tree *MIXT_Starting_Tree(t_tree *mixt_tree)
{
  ...
  if (mixt_tree->io->mod->s_opt->random_input_tree == NO)
  {
    ...
    case 2: // user-defined input tree
    {
      ...
      tree =
          Read_User_Tree(mixt_tree->io->cdata, mixt_tree->mod, mixt_tree->io);
    ...
    case 0:
    {
      ...
      tree = Dist_And_BioNJ(mixt_tree->data, mixt_tree->mod, mixt_tree->io);
      break;
    }
    default:
      assert(FALSE);
...
void MIXT_Make_Tree_For_Lk(t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    {
      tree->c_lnL_sorted = (phydbl *)mCalloc(tree->data->n_pattern, sizeof(phydbl));
      tree->cur_site_lk  = (phydbl *)mCalloc(tree->data->n_pattern, sizeof(phydbl));
      tree->old_site_lk  = (phydbl *)mCalloc(tree->data->n_pattern, sizeof(phydbl));
      tree->site_lk_cat  = (phydbl *)mCalloc(
          MAX(tree->mod->ras->n_catg, tree->mod->n_mixt_classes),
          sizeof(phydbl));
      tree->unscaled_site_lk_cat = (phydbl *)mCalloc(
          MAX(tree->mod->ras->n_catg, tree->mod->n_mixt_classes) *
              tree->data->n_pattern,
          sizeof(phydbl));
      tree->fact_sum_scale = (int *)mCalloc(tree->data->n_pattern, sizeof(int));

#if (defined(__AVX__) || defined(__SSE3__))
#ifndef WIN32
      if (posix_memalign((void **)&tree->expl, BYTE_ALIGN,
                         (size_t)2 * tree->mod->n_mixt_classes * tree->mod->ns *
                             sizeof(phydbl)))
        Generic_Exit(__FILE__, __LINE__, __FUNCTION__);
#else
      tree->expl = _aligned_malloc(2 * tree->mod->n_mixt_classes *
                                       tree->mod->ns * sizeof(phydbl),
                                   BYTE_ALIGN);
#endif
#else
      tree->expl = (phydbl *)mCalloc(
          2 * tree->mod->n_mixt_classes * tree->mod->ns, sizeof(phydbl));
#endif

      for (int i = 0; i < 2 * tree->n_otu - 1; ++i)
        Make_Edge_NNI(tree->a_edges[i]);

      tree->log_lks_aLRT = (phydbl **)mCalloc(3, sizeof(phydbl *));
      for (int i = 0; i < 3; i++)
        tree->log_lks_aLRT[i] =
            (phydbl *)mCalloc(tree->data->init_len, sizeof(phydbl));
    }
...
void MIXT_Ancestral_Sequences_One_Node(t_node *mixt_d, t_tree *mixt_tree,
                                       int print)
{
  if (mixt_d->tax)
    return;
  else
  {
    ...
    do /* For each partition element */
    {
      ...
      {
        r_mat_weight_sum = MIXT_Get_Sum_Chained_Scalar_Dbl(
            curr_mixt_tree->next->mod->r_mat_weight);
        e_frq_weight_sum = MIXT_Get_Sum_Chained_Scalar_Dbl(
            curr_mixt_tree->next->mod->e_frq_weight);
        sum_probas = MIXT_Get_Sum_Of_Probas_Across_Mixtures(
            r_mat_weight_sum, e_frq_weight_sum, curr_mixt_tree);
      }
      else
      {
        r_mat_weight_sum = 1.;
      ...
      Ns   = curr_mixt_tree->next ? curr_mixt_tree->next->mod->ns
                                  : curr_mixt_tree->mod->ns;
      NsNs = Ns * Ns;
      NsNg = Ns * curr_mixt_tree->mod->ras->n_catg;

      p = (phydbl *)mCalloc(Ns, sizeof(phydbl));

      /* for(site=0;site<curr_mixt_tree->n_pattern;site++) // For each site in
       * the current partition element */
      for (site = 0; site < curr_mixt_tree->data->init_len;
           site++) // For each site in the current partition element
      {
        ...
        do // For each class of the mixture model that applies to the current
           // partition element
        {
          if (tree->is_mixt_tree == YES)
          {
          ...
          for (catg = 0; catg < tree->mod->ras->n_catg; catg++)
          {
            for (i = 0; i < Ns; i++)
            {
              ...
              if (v0->tax)
                ...
                {
                  p0 += v0->b[0]->p_lk_tip_r[csite * Ns + j] *
                ...
                {
                  p0 +=
                      p_lk0[csite * NsNg + catg * Ns + j] *
                      Pij0[catg * NsNs + i * Ns + j] /
                      (phydbl)POW(
                          2,
                          sum_scale0[catg * curr_mixt_tree->data->n_pattern + csite]);
              ...
              if (v1->tax)
                for (j = 0; j < tree->mod->ns; j++)
                {
                  p1 += v1->b[0]->p_lk_tip_r[csite * Ns + j] *
                ...
                {
                  p1 +=
                      p_lk1[csite * NsNg + catg * Ns + j] *
                      Pij1[catg * NsNs + i * Ns + j] /
                      (phydbl)POW(
                          2,
                          sum_scale1[catg * curr_mixt_tree->data->n_pattern + csite]);
              ...
              if (v2->tax)
                for (j = 0; j < tree->mod->ns; j++)
                {
                  p2 += v2->b[0]->p_lk_tip_r[csite * Ns + j] *
                ...
                {
                  p2 +=
                      p_lk2[csite * NsNg + catg * Ns + j] *
                      Pij2[catg * NsNs + i * Ns + j] /
                      (phydbl)POW(
                          2,
                          sum_scale2[catg * curr_mixt_tree->data->n_pattern + csite]);
              ...
              p[i] += p0 * p1 * p2 * tree->mod->e_frq->pi->v[i] /
                      tree->cur_site_lk[csite] *
                      curr_mixt_tree->mod->ras->gamma_r_proba
                          ->v[tree->mod->ras->parent_class_number] *
                      tree->mod->r_mat_weight->v / r_mat_weight_sum *
                      tree->mod->e_frq_weight->v / e_frq_weight_sum /
                      sum_probas;
    ...
    */
    ...
    do
    {
      ...
      for (state = 0; state < tree->mod->ns; ++state)
      {
        ...
        {
          expl[class * 2 * ns + 2 * state] =
              POW(1. - ev * var / len, -len * len / var);
          expl[class * 2 * ns + 2 * state + 1] =
              expl[class * 2 * ns + 2 * state];
          expl[class * 2 * ns + 2 * state + 1] *=
              -(ev * rr / (1. - ev * var / len) +
                2. * len * rr * LOG(1. - ev * var / len) / var);
        }
        else
        {
    ...
    for (site = 0; site < mixt_tree->data->n_pattern; ++site)
    {
      ...
      do
      {
            ...
            tree->data->wght[tree->curr_site] > SMALL)
        {
          ...
          {
#if (defined(__AVX__))
            AVX_Lk_dLk_Core_One_Class_Eigen_Lr(dot_prod, expl ? expl : NULL, ns,
                                               lk + class, dlk + class);
#elif (defined(__SSE3__))
            SSE_Lk_dLk_Core_One_Class_Eigen_Lr(dot_prod, expl ? expl : NULL, ns,
                                               lk + class, dlk + class);
#else
            Lk_dLk_Core_One_Class_Eigen_Lr(dot_prod, expl ? expl : NULL, ns,
                                           lk + class, dlk + class);
#endif
          }
          else
          ...
          /* PhyML_Printf("\n> tree: %p class: %d lk: %f dlk: %g sum: %g site:
           * %d dot_prod[0]: %g expl[0]: %g expl[1]: %g expl[2]:
           * %g",tree,class,log(lk[class]),dlk[class],sum,site,dot_prod[0],expl[0],expl[1],expl[2]);
           */
      ...
      if (mixt_tree->mod->ras->invar == YES)
        one_m_pinv = 1. - mixt_tree->mod->ras->pinvar->v;
      else
        one_m_pinv = 1.;
      ...
      {
        if (tree->mod->ras->invar == NO &&
            mixt_tree->data->wght[tree->curr_site] > SMALL)
        {
          site_lk += lk[class] *
                     mixt_tree->mod->ras->gamma_r_proba
                         ->v[tree->mod->ras->parent_class_number] *
          ...
          /* PhyML_Printf("\n. MIXT_DLK site: %d mixt_tree: %p tree: %p class:
           * %d l: %p lk: %g dlk: %g", */
          /*              site, */
          /*              mixt_tree, */
          /*              tree, */
      ...
      if (isinf(log_site_lk) || isnan(log_site_lk))
      {
        ...
        PhyML_Fprintf(stderr, "\n. invar = %d", mixt_tree->data->invar[site]);
        PhyML_Fprintf(stderr, "\n. mixt = %d", mixt_tree->is_mixt_tree);
        PhyML_Fprintf(stderr, "\n. lk = %G log(lk) = %f < %G", site_lk,
                      log_site_lk, -BIG);
        for (class = 0; class < mixt_tree->mod->ras->n_catg; ++class)
          PhyML_Printf("\n. rr=%f p=%f",
                       mixt_tree->mod->ras->gamma_rr->v[class],
      ...
      /* PhyML_Printf("\n. site: %4d lnL: %15f dlnL:
       * %15f",site,site_lk,site_dlk); */
    }
...
// Returns the number of trees in the partition element corresponding to
// mixt_tree.
...
void MIXT_Sample_Ancestral_Seq(int fullmutmap, int fromprior, t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    {
      if (i == 0)
        class_proba = (phydbl *)mCalloc(1, sizeof(phydbl));
      else
        class_proba = (phydbl *)mRealloc(class_proba, i + 1, sizeof(phydbl));

      class_proba[i] = tree->site_lk_cat[0] *
                       loc_mixt_tree->mod->ras->gamma_r_proba
                           ->v[tree->mod->ras->parent_class_number] *
                       tree->mod->r_mat_weight->v / r_mat_weight_sum *
                       tree->mod->e_frq_weight->v / e_frq_weight_sum /
                       sum_probas;
...
void MIXT_Propagate_Tree_Update(t_tree *mixt_tree)
{
  ...
  do
  {
    ...
    {
      tree->a_nodes[i]->v[0] =
          mixt_tree->a_nodes[i]->v[0]
              ? tree->a_nodes[mixt_tree->a_nodes[i]->v[0]->num]
              : NULL;
      tree->a_nodes[i]->v[1] =
          mixt_tree->a_nodes[i]->v[1]
              ? tree->a_nodes[mixt_tree->a_nodes[i]->v[1]->num]
              : NULL;
      tree->a_nodes[i]->v[2] =
          mixt_tree->a_nodes[i]->v[2]
              ? tree->a_nodes[mixt_tree->a_nodes[i]->v[2]->num]
              : NULL;
    ...
    for (i = 0; i < 2 * mixt_tree->n_otu - 1; ++i)
    {
      if (mixt_tree->n_root == NULL ||
        ...
        Connect_One_Edge_To_Two_Nodes(
            tree->a_nodes[mixt_tree->a_edges[i]->left->num],
            tree->a_nodes[mixt_tree->a_edges[i]->rght->num],
            tree->a_edges[mixt_tree->a_edges[i]->num], tree);
      tree->a_edges[mixt_tree->a_edges[i]->num]->num =
          mixt_tree->a_edges[i]->num;
    }

    /* Necessary? Increase computation time due to large number of calls from
     * Ldsk_To_Tree function */
...
calign *MIXT_Copy_Cseq(calign *ori, option *io, t_tree *mixt_tree)
{
  ...
  cpy = Copy_Cseq(ori, io, NULL);

// Propagate to trees in the mixture
  tree = mixt_tree->next;
  do
  {
    tree->data = cpy;
...
t_tree *MIXT_Duplicate_Tree(t_tree *ori)
{
  ...
  for (i = 0; i < 2 * tree->n_otu - 1; ++i)
  {
    ...
    cpy->a_edges[i]->l_var_old =
        Duplicate_Scalar_Dbl(ori->a_edges[i]->l_var_old);
  }
...
void MIXT_Print_Site_Lk(t_tree *mixt_tree, FILE *fp)
{
  ...
  if (!mixt_tree->io->print_trace)
  {
    ...
    do
    {
      PhyML_Fprintf(fp, "# Note : P(D|M) is the probability of site D given "
                        "the model M (i.e., the site likelihood)\n");
      if (tree->mod->ras->n_catg > 1 || tree->mod->ras->invar)
      {
        ...
        PhyML_Fprintf(fp, "# of evolution rr[x], where x is the class of rate "
                          "to be considered.\n");
    ...
    do
    {
      ...
      for (site = 0; site < mixt_tree->data->init_len; site++)
      {
        ...
        PhyML_Fprintf(fp, "%-15g",
                      mixt_tree->cur_site_lk[mixt_tree->data->sitepatt[site]]);

        PhyML_Fprintf(
            fp, "%-7d",
            mixt_tree->fact_sum_scale[mixt_tree->data->sitepatt[site]]);
        ...
        if (mixt_tree->mod->ras->n_catg > 1)
        {
          ...
          do
          {
            postmean += mixt_tree->mod->ras->gamma_rr
                            ->v[tree->mod->ras->parent_class_number] *
                        tree->unscaled_site_lk_cat[tree->data->sitepatt[site]] *
                        mixt_tree->mod->ras->gamma_r_proba
                            ->v[tree->mod->ras->parent_class_number];
        ...
        if (mixt_tree->mod->ras->invar)
        {
          if ((phydbl)mixt_tree->data->invar[mixt_tree->data->sitepatt[site]] >
              -0.5)
            PhyML_Fprintf(
                fp, "%-16g",
                mixt_tree->mod->e_frq->pi->v
                    [mixt_tree->data->invar[mixt_tree->data->sitepatt[site]]]);
          else
...
void MIXT_Exchange_Nodes(t_node *a, t_node *d, t_node *w, t_node *v,
                         t_tree *mixt_tree)
{
  t_tree *tree;
  ...
  do
  {
    Exchange_Nodes(tree->a_nodes[a->num], tree->a_nodes[d->num],
                   tree->a_nodes[w->num], tree->a_nodes[v->num], tree);
    if (tree->is_mixt_tree == YES) return;
    tree = tree->next;
  } while (tree);
}
...
void MIXT_Maxfold_Cv(t_tree *mixt_tree) 
{
  ...
  do
  {
    ...
    PhyML_Fprintf(mixt_tree->io->fp_out_stats,
                  "\n. CV type: fast k-fold position (aka maxfold).");
    ...
    // for (int i = 0; i < kfold*5; ++i)
      for (int i = 0; i < kfold * 1; ++i)
      {
        ...
        c_seq_cpy->masked_pos =
            (int *)mCalloc(c_seq_cpy->n_masked, sizeof(int));
