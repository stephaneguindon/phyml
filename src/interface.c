...
void Launch_Interface(option *io)
{
  ...
  if ((io->mod->whichmodel == CUSTOMAA) && (io->datatype == AA))
  {
    ...
    fflush(NULL);
    PhyML_Printf("\n");
    PhyML_Printf("\n. Enter the rate matrix file name > ");
    fflush(NULL);
  ...
  if ((io->mod->s_opt->n_rand_starts > 0) &&
      (io->mod->s_opt->topo_search == NNI_MOVE) &&
      (io->mod->s_opt->random_input_tree))
  {
    Warn_And_Exit("\n. The random starting tree option is only compatible with "
                  "SPR based search options.\n");
  }

  if ((io->datatype == NT) && (io->mod->whichmodel > 10))
  {
    char choix;
    PhyML_Printf("\n== Err: model incompatible with the data type. Please use "
                 "JC69, K80, F81, HKY, F84, TN93 or GTR\n");
  ...
  else if ((io->datatype == AA) && (io->mod->whichmodel < 11))
  {
    char choix;
    PhyML_Printf("\n== Err: model incompatible with the data type. Please use "
                 "LG, Dayhoff, JTT, MtREV, WAG, DCMut, RtREV, CpREV, VT, "
                 "Blosum62, MtMam, MtArt, HIVw, HIVb or AB.\n");
    PhyML_Printf("\n. Type any key to exit.\n");
    if (!scanf("%c", &choix)) Exit("\n");
    Exit("\n");
  }

  if (io->mod->use_m4mod == YES)
  {
    io->mod->ns *= io->mod->m4mod->n_h;

    io->mod->m4mod = M4_Make_Light();

    M4_Init_Model(io->mod->m4mod, io->cdata, io->mod);

    M4_Make_Complete(io->mod->m4mod->n_h, io->mod->m4mod->n_o, io->mod->m4mod);

    if ((io->mod->m4mod->multipl_unscaled->optimize == YES) &&
        (io->mod->m4mod->alpha->optimize == NO))
...
void Launch_Interface_Input(option *io)
{
  ...
  PhyML_Printf("\n\n");
  PhyML_Printf("\n. Enter the tree file name > ");
  fflush(NULL);
  Getstring_Stdin(io->in_tree_file);
  io->fp_in_tree = Openfile(io->in_tree_file, 0);
  PhyML_Printf("\n");

  PhyML_Printf("\n. Enter the reference sequence file name > ");
  fflush(NULL);
...
#endif
  {
    ...
    if (choix == 'R')
      ...
      io->out_stats_file_open_mode = 2;
  }

  /* io->fp_out_stats =
   * Openfile(io->out_stats_file,io->out_stats_file_open_mode); */
...
void Launch_Interface_Data_Type(option *io)
{
  ...
  {
    io->curr_interface =
        (io->config_multigene) ? (INTERFACE_MODEL) : (INTERFACE_BRANCH_SUPPORT);
    break;
  }
  case '+':
  {
    io->curr_interface =
        (io->multigene) ? (INTERFACE_MULTIGENE) : (INTERFACE_MODEL);
    break;
  }
  case 'Y':
  {
...
void Launch_Interface_Model(option *io)
{
  ...
  if (io->datatype == NT)
  {
    ...
    {            
      ...
      if ((io->mod->whichmodel == F81) || (io->mod->whichmodel == HKY85) ||
          (io->mod->whichmodel == F84) || (io->mod->whichmodel == TN93) ||
          (io->mod->whichmodel == GTR) || (io->mod->whichmodel == CUSTOM))
      {
        PhyML_Printf("                [F] "
                     "................. Optimise equilibrium frequencies "
                     " %-15s \n",
                     (io->mod->e_frq->type == ML) ? ("yes") : ("no"));
      }
  ...
  if ((io->datatype == NT) &&
      ((io->mod->whichmodel == K80) || (io->mod->whichmodel == HKY85) ||
       (io->mod->whichmodel == F84) || (io->mod->whichmodel == TN93)))
  {
    strcpy(s, (io->mod->kappa->optimize) ? ("estimated") : ("fixed"));
    if (io->mod->kappa->optimize)
      strcat(s, "");
    else
      strcat(s, " (ts/tv = ");

    /*       (io->mod->kappa->optimize)?((char *)strcat(s,"")):((char
     * *)sprintf(s+(int)strlen((char *)s),"%3.2f)",io->mod->kappa->v)); */
    if (io->mod->kappa->optimize)
    {
      strcat((char *)s, "");
    }
  ...
  if (io->mod->ras->pinvar->optimize == YES)
  {
    strcat(s, "");
  }
  else
  ...
  {
    strcpy(s, (io->mod->ras->alpha->optimize) ? ("estimated") : ("fixed"));
    strcat(s, io->mod->ras->alpha->optimize ? "" : " (alpha = ");

    if (io->mod->ras->alpha->optimize)
    {
      strcat(s, "");
    }
    else
  ...
  case 'O':
  {
    io->mod->r_mat->optimize = (io->mod->r_mat->optimize) ? NO : YES;
    break;
  }
  ...
  case 'K':
  {
    ...
    if (io->mod->whichmodel == CUSTOM)
    {
      ...
      for (i = 0; i < io->mod->r_mat->n_diff_rr; i++)
      {
        sprintf(rr, "\n. [");
        for (j = 0; j < 6; j++)
        {
          if (io->mod->r_mat->rr_num->v[j] == i)
          {
        ...
        PhyML_Printf("  (current=%.2f) > ", io->mod->r_mat->rr_val->v[i]);
        ...
        if (rr[0] != '\0')
        {
          ...
          {
            if (++n_trial > 10)
              Exit("\n== Err : the value of this parameter must be a positive "
                   "number\n");
            PhyML_Printf(
  ...
  case 'E':
  {
    ...
    if (io->mod->whichmodel == CUSTOM)
    {
      ...
      if (io->mod->e_frq->type == USER)
      {
        ...
        if (sum > 1.0 || sum < 1.0)
        {
          PhyML_Printf("\n. The nucleotide frequencies have to be normalised "
                       "in order to sum to 1.0.\n");
          PhyML_Printf("\n. The frequencies are now : f(A)=%f, f(C)=%f, "
                       "f(G)=%f, f(T)=%f.\n",
  ...
  case 'A':
  {
    ...
    if (io->mod->ras->alpha->optimize == NO)
    {
      PhyML_Printf("\n. Optimise alpha ? [Y/n] ");
  ...
  case 'V':
  {
    ...
    if(io->mod->ras->pinvar->optimize == NO)
    {
    ...
    {
      io->mod->s_opt->opt_subst_param = 1;
      io->mod->ras->pinvar->optimize  = YES;
    ...
    case 'n':
    {
      ...
      {
        if (++n_trial > 10)
          Exit("\n== Err : the proportion of invariable sites must be a "
               "positive number between 0.0 and 1.0");
        PhyML_Printf("\n. The proportion must be a positive number between 0.0 "
                     "and 1.0\n");
        PhyML_Printf("\n. Enter a new value > ");
        Getstring_Stdin(p);
      }
  ...
  case 'T':
  {
    ...
    if ((io->datatype == AA) || (io->mod->whichmodel == JC69) ||
        (io->mod->whichmodel == F81) || (io->mod->whichmodel == GTR) ||
        (io->mod->whichmodel == CUSTOM))
    {
    ...
    case 'y':
    {
      io->mod->kappa->v               = 4.0;
      io->mod->s_opt->opt_subst_param = 1;
      io->mod->kappa->optimize        = 1;
      if (io->mod->whichmodel == TN93) io->mod->lambda->optimize = 1;
      break;
    }
    case 'N':
    case 'n':
  ...
  case '+':
  {
    io->curr_interface = (io->config_multigene) ? (INTERFACE_DATA_TYPE)
                                                : (INTERFACE_TOPO_SEARCH);
    break;
  }
  case 'Y':
  {
  ...
  if (io->mod->ras->alpha->optimize == YES || io->mod->kappa->optimize == YES ||
      io->mod->lambda->optimize == YES || io->mod->ras->pinvar->optimize == YES||
      io->mod->r_mat->optimize == YES)
...
void Launch_Interface_Topo_Search(option *io)
{
  ...
  else
  {
    PhyML_Printf("                [L] "
                 ...
                 " %-15s \n",
                 (io->mod->s_opt->opt_bl_one_by_one) ? ("yes") : ("no"));
  }
  ...
  case 'N':
  {
    ...
    {
      if (++n_trial > 10)
        Exit("\n== Err : the number of starting trees must be a positive "
             "integer\n");
      PhyML_Printf(
  ...
  case 'L':
  {
    if (!io->mod->s_opt->opt_topo)
    {
      io->mod->s_opt->opt_bl_one_by_one = (io->mod->s_opt->opt_bl_one_by_one) ? (0) : (1);
    }
    break;
  }
  ...
  {
    io->mod->s_opt->random_input_tree =
        (io->mod->s_opt->random_input_tree) ? (0) : (1);
...
void Launch_Interface_Branch_Support(option *io)
{
  ...
  case 'B':
  {
    ...
    else
    {
      ...
      {
        if (++n_trial > 10)
          Exit("\n== Err : the number of replicates must be a positive "
               "integer\n");
...
void Launch_Interface_Multigene(option *io)
{
  ...
  if ((io->n_data_sets > 1) && (io->multigene))
  {
    ...
    for (set = 0; set < io->n_part; set++)
    {
      ...
      PhyML_Printf("\n. Enter the sequence file name [data set %2d] > ",
                   set + 1);
      fflush(NULL);
      Getstring_Stdin(io->st->optionlist[set]->in_align_file);
      io->st->optionlist[set]->fp_in_align =
          Openfile(io->st->optionlist[set]->in_align_file, 0);
    }
