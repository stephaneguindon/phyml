...
int MIGREP_Main_Estimate(int argc, char *argv[])
{
  ...
  tree->mmod->sigsq = MIGREP_Update_Sigsq(tree);
...
void MIGREP_Simulate_Backward_Core(int new_loc, t_dsk *init_disk, t_tree *tree)
{
  ...
  if(new_loc == YES)
    {
      init_disk = MIGREP_Make_Disk_Event(n_dim,tree->n_otu);
      MIGREP_Init_Disk_Event(init_disk,n_dim,NULL);
      init_disk->time             = 0.0;
      init_disk->mmod             = mmod;
  ...
  init_disk->prev = MIGREP_Make_Disk_Event(n_dim,tree->n_otu);
  MIGREP_Init_Disk_Event(init_disk->prev,n_dim,NULL);
  ...
  do
    {      
      ...
      disk->prev = MIGREP_Make_Disk_Event(n_dim,tree->n_otu);
...
phydbl MIGREP_Lk(t_tree *tree)
{
  ...
  /* if(tree->mmod->update_rad == YES) tree->mmod->rad = MIGREP_Update_Radius(tree); */
            ...
            case MIGREP_NORMAL:
              {
                ...
                /* log_dens_coal = -LOG(SQRT(2.*PI*POW(mmod->rad,2))); */
                /* For(j,mmod->n_dim) log_dens_coal += -POW(disk->prev->ldsk->coord->lonlat[j] - disk->prev->centr->lonlat[j],2)/(2.*POW(mmod->rad,2)); */
                lnL += log_dens_coal;
                break;
              }
            }
...
phydbl *MIGREP_MCMC(t_tree *tree)
{
  ...
  tree->mmod->lbda = Uni()*(10. - 1.) + 1.;
  tree->mmod->mu   = Uni()*(1.0 - 0.6) + 0.6;
  ...
  do
    {
      ...
      if(!(tree->mcmc->run%tree->mcmc->sample_interval))
        {
          ...
          PhyML_Fprintf(fp_stats,"\n%6d\t%9.1f\t%9.1f\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%6.3f\t%6d\t%6d\t%6d\t%6.2f\t%6.2f\t%8.1f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f\t%6.2f",
                        tree->mcmc->run,
                        tree->c_lnL,
                        tree->mmod->c_lnL,
                        tree->mmod->lbda,
                        tree->mcmc->mode[tree->mcmc->num_move_migrep_lbda],
                        tree->mmod->mu,
                        tree->mcmc->mode[tree->mcmc->num_move_migrep_mu],
                        MIGREP_Update_Sigsq(tree),
                        tree->mcmc->mode[tree->mcmc->num_move_migrep_sigsq],
          ...
          res[4 * tree->mcmc->chain_len / tree->mcmc->sample_interval +  tree->mcmc->run / tree->mcmc->sample_interval] = disk->ldsk->coord->lonlat[0];
          res[5 * tree->mcmc->chain_len / tree->mcmc->sample_interval +  tree->mcmc->run / tree->mcmc->sample_interval] = disk->ldsk->coord->lonlat[1];
          ...
          PhyML_Fprintf(fp_summary,"\n# SampArea\t TrueLbda\t TrueMu\t TrueSig\t TrueNeigh\t Diversity\t TrueInt\t TrueCoal\t TrueHits\t RegNeigh\t TrueXroot\t TrueYroot\t Lbda5\t Lbda50\t Lbda95\t LbdaMod \t Mu5\t Mu50\t Mu95\t  MuMod \t Sig5\t Sig50\t Sig95\t SigMod \t Neigh5\t Neigh50\t Neigh95\t NeighMod \t Xroot5\t Xroot50\t Xroot95\t Yroot5\t Yroot50\t Yroot95\t ESSLbda \t ESSMu \t ESSSig");
      ...
      if(tree->mcmc->sample_num > 1E+2                             &&
         tree->mcmc->ess[tree->mcmc->num_move_migrep_lbda]  > 1000. &&
         tree->mcmc->ess[tree->mcmc->num_move_migrep_mu]    > 1000. &&
         tree->mcmc->ess[tree->mcmc->num_move_migrep_sigsq] > 1000.) break;
...
{
  MIGREP_Update_Lindisk_List_Pre(tree->disk->prev,tree);
}
...
void MIGREP_Update_Lindisk_List_Pre(t_dsk *disk, t_tree *tree)
{
  if(!disk) return;
    ...
    {
      MIGREP_Update_Lindisk_List_Core(disk,tree);
      MIGREP_Update_Lindisk_List_Pre(disk->prev,tree);    
    }
}
...
phydbl MIGREP_Update_Radius(t_tree *tree)
{
  switch(tree->mmod->name)
    ...
    case MIGREP_NORMAL:  
      { 
        return(POW(tree->mmod->sigsq/(MIGREP_Rate_Per_Unit_Area(tree)*4.*PI*tree->mmod->mu),0.25));
        break; 
      }
    }
  return(-1.);
